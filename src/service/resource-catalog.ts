...
import type { InstalledExtensionPackage } from "../extensions/packages.js";
import {
  ...
  for (const value of packages) {
    ...
    if (entry.project !== undefined) {
      ...
      const declaration = parseProjectPackageDeclaration({
        schemaVersion: 1,
        packages: [{ id: entry.id, source: project.source, disabledResources: project.disabledResources }],
      });
      parseProjectPackageLock({
        schemaVersion: 1,
        declarationSha256: projectPackageDeclarationSha256(declaration),
        packages: [{ id: entry.id, declaration: declaration.packages[0], resolved: project.resolved }],
      });
    }
  }
