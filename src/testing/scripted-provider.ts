...
function validateUsage(value: unknown, label: string): asserts value is NormalizedUsage {
  ...
  if (usage.cost !== undefined) boundedString(usage.cost, `${label}.cost`, { maxBytes: 1_024 });
...
function validateState(value: unknown, label: string): asserts value is ProviderState {
  ...
  switch (kind) {
    ...
    case "mistral_conversations":
      ...
      boundedString(state.requestFingerprint, `${label}.requestFingerprint`, { maxBytes: 256 });
...
function validateEvent(value: unknown, label: string): asserts value is AdapterEvent {
  ...
  switch (event.type) {
    ...
    case "tool_call_delta":
      ...
      break;
    case "tool_call_end":
      exactKeys(event, ["type", "index", "name", "rawArguments", "id", "arguments", "parseError"], label);
