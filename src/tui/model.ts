...
import type { TranscriptEntry, TuiContext, TuiLimits, TuiUsageSummary } from "./types.js";
...
const USAGE_NUMBERS = [
  "inputTokens", "outputTokens", "totalTokens", "cacheReadTokens", "cacheWriteTokens", "reasoningTokens", "serverToolCalls", "durationMs",
...
function addUsage(left: NormalizedUsage | undefined, right: NormalizedUsage): NormalizedUsage {
  ...
  for (const key of USAGE_NUMBERS) {
    ...
    if (first !== undefined || second !== undefined) total[key] = (first ?? 0) + (second ?? 0);
  }
  ...
  if ((firstCost !== undefined && Number.isFinite(firstCost)) || (secondCost !== undefined && Number.isFinite(secondCost))) {
    total.cost = String((Number.isFinite(firstCost) ? firstCost! : 0) + (Number.isFinite(secondCost) ? secondCost! : 0));
  }
  return total;
}
...
export class TuiModel {
    ...
    switch (event.type) {
      ...
      case "usage":
        {
          ...
          const current = event.semantics === "incremental" ? addUsage(prior, event.usage) : { ...event.usage };
          ...
          const aggregate = [...this.#usageByRun.values()].reduce<NormalizedUsage>((total, value) => addUsage(total, value), {});
      ...
      case "extension_state":
        this.#append({
          ...
          extension: {
            type: "state",
            extensionId: sanitizeTerminalText(event.extensionId),
            schemaVersion: event.schemaVersion,
            key: sanitizeTerminalText(event.key),
          },
        });
        break;
      case "extension_message":
        if (event.transcript !== false) {
          this.#append({
            id: envelope.eventId,
            kind: "status",
            ...
            extension: {
              ...
              extensionId: sanitizeTerminalText(event.extensionId),
              schemaVersion: event.schemaVersion,
              key: sanitizeTerminalText(event.kind),
            },
          });
        }
        break;
  ...
  #appendMessage(message: CanonicalMessage): void {
    ...
    if (message.role === "assistant") this.#mutableEntryIds.add(message.id);
  }
