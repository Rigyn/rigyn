...
interface RetainedSessionEvent {
  branch: string;
  envelope: EventEnvelope<ExtensionStateEvent | ExtensionMessageEvent>;
  bytes: number;
}

function retainSessionEvent(
  envelope: EventEnvelope<ExtensionStateEvent | ExtensionMessageEvent>,
  branch: string,
): RetainedSessionEvent {
  const event = envelope.event.type === "extension_state"
    ? canonicalExtensionStateEvent(envelope.event)
    : canonicalExtensionMessageEvent(envelope.event);
  const snapshot: EventEnvelope<ExtensionStateEvent | ExtensionMessageEvent> = {
  ...
  const metadataBytes = Buffer.byteLength(
    `${snapshot.eventId}\0${snapshot.threadId}\0${snapshot.runId ?? ""}\0${snapshot.parentEventId ?? ""}\0${snapshot.timestamp}\0${branch}`,
    "utf8",
  );
  return {
    branch,
    envelope: snapshot,
    bytes: metadataBytes + Buffer.byteLength(JSON.stringify(event), "utf8") + 64,
  };
}
...
export class TuiController {
  ...
  readonly #onAction: ((action: TuiAction) => void) | undefined;
  ...
  renderExtensionSession(
    envelope: EventEnvelope<ExtensionStateEvent | ExtensionMessageEvent>,
    branch: string,
  ): void {
    this.#ensureStarted();
    if (envelope.event.type !== "extension_state" && envelope.event.type !== "extension_message") {
      throw new Error("Extension session rendering requires an extension event");
    }
    if (
      ...
      throw new Error("Extension session rendering requires a branch");
    }
    if (envelope.event.type === "extension_state" || envelope.event.transcript !== false) {
      const retained = retainSessionEvent(envelope, branch);
      const prior = this.#sessionEvents.get(envelope.eventId);
      if (prior !== undefined) this.#sessionEventBytes -= prior.bytes;
  ...
  #pruneSessionEvents(): void {
    ...
    for (const [eventId, retained] of this.#sessionEvents) {
      if (live.has(eventId)) continue;
      this.#sessionEvents.delete(eventId);
      this.#sessionEventBytes -= retained.bytes;
    }
    while (
      this.#sessionEvents.size > this.#limits.maxTranscriptEntries ||
      this.#sessionEventBytes > this.#limits.maxTranscriptBytes
    ) {
      const oldest = this.#sessionEvents.entries().next().value as [string, RetainedSessionEvent] | undefined;
      if (oldest === undefined) break;
      this.#sessionEvents.delete(oldest[0]);
      this.#sessionEventBytes -= oldest[1].bytes;
    }
  }
  ...
  #renderToolBlocks(entries: readonly TranscriptEntry[], width: number, height: number): Map<string, ToolRenderSlots> {
    ...
    for (const entry of entries) {
      ...
      const call = invoke(() => owner.binding.renderCall(name, view, selectedContext));
      const result = renderedResult === undefined
        ? undefined
        : invoke(() => owner.binding.renderResult(name, view, selectedContext));
      if (call !== undefined || result !== undefined) blocks.set(entry.callId, {
        ...(call === undefined ? {} : { call }),
  ...
  #renderSessionBlocks(entries: readonly TranscriptEntry[], width: number, height: number): Map<string, RuntimeUiBlock> {
    ...
    for (const entry of entries) {
      ...
      const retained = this.#sessionEvents.get(entry.id);
      if (retained === undefined || owner.signal.aborted || this.#sessionRenderers !== owner) continue;
      try {
        const value = retained.envelope.event.type === "extension_state"
          ? owner.binding.renderState(
              retained.envelope as EventEnvelope<ExtensionStateEvent>,
              retained.branch,
              context,
            )
          : retained.envelope.event.transcript === false
            ? undefined
            : owner.binding.renderMessage(
                retained.envelope as EventEnvelope<ExtensionMessageEvent>,
                retained.branch,
                context,
              );
        if (value === undefined || owner.signal.aborted || this.#sessionRenderers !== owner) continue;
        blocks.set(entry.id, sanitizeRuntimeUiBlock(value, { width }));
      } catch {
  ...
  #handleTreeOverlayKey(overlay: Overlay, event: KeyEvent): boolean {
    ...
    if (this.#keybindings.matches("app.tree.foldOrPreviousEndpoint", event)) {
      const selected = overlay.items[overlay.selected];
    ...
    if (this.#keybindings.matches("app.tree.unfoldOrNextEndpoint", event)) {
      const selected = overlay.items[overlay.selected];
