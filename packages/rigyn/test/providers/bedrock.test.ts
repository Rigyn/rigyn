...
const requestDuplex = (request: Request): string | undefined =>
  (request as Request & { duplex?: string }).duplex;
  ...
  for (const entry of [
    ...
    await t.test(entry.name, async () => {
      ...
      if (entry.wireDisplay !== undefined) {
        wire.register("bedrock", {
          interceptRequest(observed) {
            ...
            const requestFields = (body as Record<string, unknown>).additionalModelRequestFields as Record<string, unknown>;
            const thinking = requestFields.thinking as Record<string, unknown>;
      ...
      const adapter = new BedrockAdapter({
        region: entry.region,
        signer: (unsigned) => unsigned,
        ...(entry.thinkingDisplay === undefined ? {} : { thinkingDisplay: entry.thinkingDisplay }),
        ...(entry.wireDisplay === undefined ? {} : { wire }),
...
test("Bedrock emits strict tool specifications only when the model advertises support", async () => {
  ...
  const tool = ((captured[0]?.toolConfig as { tools?: Array<{ toolSpec?: Record<string, unknown> }> })
    .tools?.[0]?.toolSpec);
...
test("Bedrock recognizes Opus 5 capabilities through model IDs and application profile names", async () => {
  ...
  for (const [index, effort] of ["max", "xhigh"].entries()) {
    ...
    assert.deepEqual(
      (captured[index]?.messages as Array<{ content: unknown[] }>).at(-1)?.content.at(-1),
      { cachePoint: { type: "default" } },
    );
  }
});
