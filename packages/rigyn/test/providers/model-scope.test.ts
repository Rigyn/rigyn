...
test("literal model IDs containing glob syntax resolve before pattern expansion", () => {
  const available: ScopedModelSelection[] = [
    { provider: "cloud", model: "model[preview]" },
    { provider: "cloud", model: "modelp" },
    { provider: "cloud", model: "model[preview]:high" },
  ];
  assert.deepEqual(orderModelsForScope(available, ["cloud/model[preview]"]), [
    { provider: "cloud", model: "model[preview]" },
  ]);
  assert.deepEqual(orderModelsForScope(available, ["cloud/model[preview]:high"]), [
    { provider: "cloud", model: "model[preview]:high" },
  ]);
  assert.deepEqual(
    orderModelsForScope(available.filter((entry) => entry.model !== "model[preview]:high"), [
      "cloud/model[preview]:high",
    ]),
    [{ provider: "cloud", model: "model[preview]", reasoningEffort: "high" }],
  );
});
