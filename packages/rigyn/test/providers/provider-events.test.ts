...
test("Anthropic subscription credentials use bearer OAuth compatibility headers", async () => {
  ...
  assert.equal(headers?.get("user-agent"), "rigyn/0.6.0");
...
test("OpenAI-compatible chat streams reasoning_text deltas", async () => {
  const response = sse({
    id: "chat-reasoning-text",
    model: "test-model",
    choices: [{
      index: 0,
      delta: { reasoning_text: "private trace", content: "done" },
      finish_reason: "stop",
    }],
  }) + "data: [DONE]\n\n";
  const adapter = new OpenAICompatibleAdapter({
  ...
  assert.deepEqual(
    ...
    [{ type: "reasoning_delta", part: 0, text: "private trace", visibility: "provider_trace" }],
  );
});
