...
test.afterEach(async () => {
  ...
  delete (globalThis as Record<string, unknown>).__compatLoaderInspections;
...
test("compat package discovery bounds manifests at exactly 1 MiB", async () => {
  const cwd = await temporaryDirectory("rigyn-compat-bounded-");
  const agentDir = await temporaryDirectory("rigyn-compat-agent-");
  const roots = ["exact", "oversized"].map((name) => join(cwd, name));
  const manifest = JSON.stringify({ rigyn: { extensions: ["declared.js"] } });
...
`);
  ...
  try {
    ...
    assert.equal((globalThis as Record<string, unknown>).__compatLoaderInspections, undefined);
