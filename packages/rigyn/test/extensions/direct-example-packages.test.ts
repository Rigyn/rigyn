  ...
  host.setDirectActionsHandler({
    ...
    async exec(...values) {
      ...
      if (name === "subprocess-workers") {
        ...
        const policyIndex = args.indexOf("--append-system-prompt");
        const specialistPolicy = policyIndex < 0 ? "" : args[policyIndex + 1] ?? "";
        if (specialistPolicy.includes("reviewer specialist")) {
          return { stdout: "", stderr: "review worker unavailable", code: 7, killed: false };
        }
        const role = specialistPolicy.match(/the (\w+) specialist/u)?.[1] ?? "unknown";
        return {
...
test("lifecycle example observes the complete run lifecycle", async (context) => {
  ...
  await host.dispatch("tool_execution_update", { toolCallId: "call-1", toolName: "read", partialResult: {} } as never);
  await host.dispatch("tool_execution_end", { toolCallId: "call-1", toolName: "read", result: {}, isError: false } as never);
...
test("state and policy example persists bounded workspace memory and tasks and blocks protected paths", async (context) => {
  ...
  assert.ok(paths);
  const saved = JSON.parse(await readFile(join(paths.workspace, "workspace-state.json"), "utf8")) as {
    memories: unknown[];
    tasks: unknown[];
  };
...
test("subprocess workers discover agents and cover command, single, parallel, chain, progress, usage, and cancellation", async (context) => {
  ...
  const processNotices: string[] = [];
  await runCommand(host, "example-process", "managed output", processNotices);
  assert.deepEqual(processNotices, ["managed output"]);
  assert.equal(calls.filter((entry) => entry.name === "exec").length, 0);
  const notices: string[] = [];
  ...
  for (const call of executions) {
    ...
    assert.equal(args.at(-4), "low");
    assert.equal(args.at(-3), "--append-system-prompt");
    const policy = args.at(-2)!;
    assert.match(policy, /bounded delegated run/u);
    assert.doesNotMatch(policy, /\$\(not-a-shell\)/u);
    policies.add(policy);
    assert.equal(args.at(-1), "Task: $(not-a-shell)");
...
test("session lifecycle example observes guards and delegates tree navigation and compaction", async (context) => {
  ...
  assert.ok(navigation.values.at(-1) instanceof AbortSignal);
  assert.equal((navigation.values.at(-1) as AbortSignal).aborted, false);
