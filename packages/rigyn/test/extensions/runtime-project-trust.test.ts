...
test("project trust is limited, diagnostic on errors, and first decision wins", async (context) => {
  ...
  const entries = [
    ...
    await sourceEntry(root, "advisory", `export default (api) => api.on("project_trust", (event, context) => {
      globalThis.__rigynCapturedTrustUi = context.ui;
  ...
  const capturedUi = (globalThis as Record<string, unknown>).__rigynCapturedTrustUi as
    RuntimeProjectTrustListenerContext["ui"];
  ...
  delete (globalThis as Record<string, unknown>).__rigynCapturedTrustUi;
