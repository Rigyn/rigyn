  ...
  const observe = (envelope: EventEnvelope): void => {
    ...
    if (event.type === "assistant_started") observation.steps = Math.max(observation.steps, event.step);
      ...
      const eventType = (
        ...
        && typeof event.details.type === "string"
      ) ? event.details.type : undefined;
...
test("credential-gated live rigyn dogfood", { skip: !ENABLED, timeout: SUITE_TIMEOUT_MS }, async (context) => {
  ...
  await context.test("authors and survives refresh of a fresh structured-tool extension", {
    ...
    try {
      const observation = await runLiveAgent(scenario, runtime, model, {
        ...
        prompt: [
          "Use the advertised rigyn-dev skill to create a fresh package at dogfood-extension/.",
          "First read the rigyn-dev SKILL.md and its extensions reference, then read the extension/package documentation it routes to and inspect only the focused starter and tool-rendering examples.",
      ...
      "agent did not load the bundled rigyn-dev skill");
      assert.equal(inspected(extensionSkillReference), true,
      "agent did not read the bundled rigyn-dev extension reference");
