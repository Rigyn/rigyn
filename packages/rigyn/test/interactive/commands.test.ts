...
test("interactive command registry owns names, aliases, visibility, and active policy", () => {
  ...
  for (const name of ["model", "thinking", "llama", "settings", "compact", "resume", "fork", "tree", "login", "logout", "quit", "prompt", "skill"]) {
...
test("palette and help are generated from visible registry metadata", () => {
  ...
  assert.match(
    palette.find((item) => item.value === "/reload")?.label ?? "",
    /keybindings, extensions, skills, prompts, themes, and context files/u,
  );
