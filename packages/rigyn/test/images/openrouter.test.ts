...
test("OpenRouter image retries keep an unlimited huge server delay pending until cancellation", async () => {
  ...
  const state: FakeState = {
    ...
    run: async () => {
      attempts += 1;
      const error = new Error("wait a very long time") as Error & { status: number; headers: Headers };
      error.status = 429;
      error.headers = new Headers({ "retry-after-ms": String(Number.MAX_SAFE_INTEGER) });
      throw error;
    },
  };
  const generate = createOpenRouterImagesGenerator({ loadSdk: async () => fakeSdk(state) });
    ...
    run: async () => successfulResponse({
      usage: {
        ...
        prompt_tokens_details: {
          cached_tokens: 0,
          ...(includeWrite ? { cache_write_tokens: 0 } : {}),
        },
      },
    }),
  };
