...
test("the published theme schema matches the runtime token and role contracts", async () => {
  const schema = JSON.parse(await readFile(new URL("../../resources/schemas/theme-v1.json", import.meta.url), "utf8")) as {
    $defs: {
      themeName: { not: { enum: string[] } };
      semanticColors: {
        propertyNames: { enum: string[] };
        allOf: Array<{ required: string[] }>;
      };
      roleStyles: { propertyNames: { enum: string[] } };
    };
  };
  ...
  const requiredTokens = schema.$defs.semanticColors.allOf.flatMap((group) => group.required);
  ...
  delete (colorsWithoutFallbackTokens as Partial<Record<ThemeToken, string | number>>).thinkingMax;
  delete (colorsWithoutFallbackTokens as Partial<Record<ThemeToken, string | number>>).scrollbarThumb;
...
test("signal makes operational states distinct while retaining non-color glyphs", () => {
  ...
  assert.equal(signal.getBgAnsi("toolSuccessBg"), "\u001b[48;5;22m");
