...
import type { AdapterEvent, CanonicalMessage, ModelInfo, ProviderAdapter, ProviderRequest, ProviderState } from "../../src/core/types.js";
  ...
  const provider = new ScriptedProvider([
    () => events([
      ...
      },
      {
        type: "response_end",
        reason: "tool_calls",
        state: { kind: "gateway_messages", assistantContent: [] },
    ...
    () => events([
      ...
      { type: "text_delta", part: 0, text: "done" },
      { type: "response_end", reason: "stop", state: { kind: "gateway_messages", assistantContent: [] } },
    ]),
  ]);
...
test("tool calls from a length-truncated provider response are never executed", async () => {
  ...
  assert.ok(streamedIndex < requestedIndex && requestedIndex < completedIndex);
...
test("provider response identity is single-shot, bounded, and control-free", async (t) => {
  for (const entry of [
    ...
    },
    {
      ...
      pattern: /streaming tool call arguments/u,
    },
    {
      ...
      pattern: /streaming tool call parse error/u,
    },
    {
      ...
      pattern: /oversized streaming tool call arguments/u,
    },
  ]) {
