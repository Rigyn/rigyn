...
test("refresh composes direct extensions, inline factories, resources, context, and prompt files", async (t) => {
  ...
  const projectSystem = join(value.cwd, ".rigyn", "SYSTEM.md");
  const projectAppend = join(value.cwd, ".rigyn", "APPEND_SYSTEM.md");
...
test("trust bootstrap reapplies project, user, and inline registration precedence without reactivation", async (t) => {
  ...
  const rendererDisposalKey = `__rigynPrecedenceRendererDisposals${Date.now()}${Math.random().toString(16).slice(2)}`;
        ...
        api.registerTool({
          ...
          renderCall() {
            return {
              ...
              invalidate() {},
              dispose() {
                const values = ((globalThis as Record<string, unknown>)[rendererDisposalKey] ?? {}) as Record<string, number>;
                values.inline = (values.inline ?? 0) + 1;
                (globalThis as Record<string, unknown>)[rendererDisposalKey] = values;
              },
            };
          },
          parameters: { type: "object", properties: {}, additionalProperties: false },
  ...
  t.after(async () => {
    await extensionHost(loader.getExtensions()).close();
    delete (globalThis as Record<string, unknown>)[activationKey];
    delete (globalThis as Record<string, unknown>)[rendererDisposalKey];
  });
  ...
  await host.close();
  assert.deepEqual((globalThis as Record<string, unknown>)[rendererDisposalKey], { project: 1 });
});
...
test("refresh evaluates every changed MJS factory generation from fresh source bytes", async (t) => {
  ...
  const activationKey = `__rigynMjsRefresh${Date.now()}${Math.random().toString(16).slice(2)}`;
...
test("refresh adopts a prepared trust host without activating its factories twice", async (t) => {
  ...
  const activationKey = `__rigynPreparedActivation${Date.now()}${Math.random().toString(16).slice(2)}`;
  await writeFile(extension, `export default (api) => {
    globalThis[${JSON.stringify(activationKey)}] = (globalThis[${JSON.stringify(activationKey)}] ?? 0) + 1;
