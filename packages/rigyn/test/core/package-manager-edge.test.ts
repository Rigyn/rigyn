...
test("shared skill roots stay user-scoped when the trusted workspace is home", async () => {
  const value = await fixture();
  const home = join(value.root, "home");
  ...
  for (const path of skillPaths) {
    const matches = result.skills.filter((entry) => entry.path === path);
    assert.equal(matches.length, 1);
    assert.equal(matches[0]?.metadata.scope, "user");
  }
});

test("a relative home cannot make untrusted workspace skills personal", async () => {
  const value = await fixture();
  ...
  const packages = new DefaultPackageManager({
    ...
    homeDirectory: relative(process.cwd(), value.cwd),
  });
