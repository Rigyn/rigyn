...
test("CLI help has concise global and command-specific surfaces", () => {
  ...
  assert.match(global, /^rigyn \S+ — agent harness/u);
  ...
  assert.match(renderCliHelp("serve"), /RIGYN_SERVE_TOKEN/u);
  ...
  assert.match(renderCliHelp("self-update"), /latest verified rigyn\s+GitHub release/u);
...
test("the exact version command remains available during an installer lifecycle transaction", async (context) => {
  const root = await mkdtemp(join(tmpdir(), "rigyn-version-lifecycle-"));
