...
test("shared personal skill roots at home stay user-scoped without a project prompt", async (context) => {
  const root = await mkdtemp(join(tmpdir(), "rigyn-project-trust-home-"));
  context.after(() => rm(root, { recursive: true, force: true }));
  ...
  const resolver = new ProjectTrustResolver(
    ...
    { terminal, homeDirectory: home },
  );

  assert.deepEqual(await discoverProjectTrustResources(home, home), []);
  assert.equal(await resolver.isTrusted(home), false);
  assert.deepEqual(terminal.prompts, []);

  await mkdir(join(home, ".rigyn"), { recursive: true });
  await writeFile(join(home, ".rigyn", "config.json"), "{}");
  ...
  const homeProjectResolver = new ProjectTrustResolver(
    ...
    { terminal: homeProjectTerminal, homeDirectory: home },
  );
  ...
  assert.deepEqual(await discoverProjectTrustResources(project, home), [
    ".agents/skills",
    ".claude/skills",
    ".codex/skills",
  ]);
  ...
  assert.deepEqual(await discoverProjectTrustResources(project, relative(process.cwd(), project)), [
    "../.agents/skills",
    ".agents/skills",
    ".claude/skills",
    ".codex/skills",
...
test("the Rigyn home cannot become project scope even through a stale saved decision", async (context) => {
  const root = await mkdtemp(join(tmpdir(), "rigyn-project-trust-agent-root-"));
  context.after(() => rm(root, { recursive: true, force: true }));
  const home = join(root, "home");
  ...
  const terminal = new SelectingTerminal(/Enable this workspace/u);
  const resolver = new ProjectTrustResolver(store, { terminal, homeDirectory: home, agentDirectory });

  assert.deepEqual(await discoverProjectTrustResources(home, home, agentDirectory), []);
  assert.equal(await resolver.isTrusted(home), false);
  assert.deepEqual(terminal.prompts, []);

  const separateAgentDirectory = join(root, "custom-rigyn-home");
  assert.deepEqual(await discoverProjectTrustResources(home, home, separateAgentDirectory), [
    ".rigyn/config.json",
    ".rigyn/extensions",
  ]);
  ...
  const customResolver = new ProjectTrustResolver(
    ...
    { terminal: customTerminal, homeDirectory: home, agentDirectory: separateAgentDirectory },
  );
  ...
  try {
    ...
    assert.equal(hasTrustRequiringProjectResources(home, home), false);
    process.env.RIGYN_HOME = separateAgentDirectory;
    assert.equal(hasTrustRequiringProjectResources(home, home), true);
  } finally {
...
test("nested launches discover repository-root shared skills before trust is decided", async (context) => {
  const root = await mkdtemp(join(tmpdir(), "rigyn-project-trust-nested-"));
  context.after(() => rm(root, { recursive: true, force: true }));
...
test("non-repository launches use the same ancestor skill boundary for trust and loading", async (context) => {
  const root = await mkdtemp(join(tmpdir(), "rigyn-project-trust-no-repository-"));
  context.after(() => rm(root, { recursive: true, force: true }));
  const home = join(root, "home");
  ...
  assert.deepEqual(await discoverProjectTrustResources(workspace, home), [
    "../../.agents/skills",
  ]);
  assert.equal(hasTrustRequiringProjectResources(workspace, home), true);
...
test("ancestor skill links and unreadable directories remain trust-gated", {
  skip: process.platform === "win32" ? "POSIX permission and symlink behavior" : false,
}, async (context) => {
  const root = await mkdtemp(join(tmpdir(), "rigyn-project-trust-unsafe-ancestor-"));
  ...
  assert.deepEqual(await discoverProjectTrustResources(workspace, join(root, "home")), [
    "../.agents/skills",
  ]);
...
test("resource discovery is metadata-only and covers settings, package declarations, prompts, extensions, and compatible skills", async (context) => {
  ...
  await writeFile(join(value.workspace, ".rigyn", "config.json"), "not parsed during discovery");
...
test("workspace approval persists once and revocation is rechecked without a second prompt", async (context) => {
  ...
  await writeFile(join(value.workspace, ".rigyn", "config.json"), "{}");
  const terminal = new SelectingTerminal(/Enable this workspace/u);
  const resolver = new ProjectTrustResolver(value.store, { terminal });
...
test("project trust events run once per target workspace and retain the launch cwd", async (context) => {
  ...
  await writeFile(join(value.workspace, ".rigyn", "config.json"), "{}");
  await writeFile(join(second, ".rigyn", "config.json"), "{}");
...
test("pre-trust user extensions are handed off once and project extensions append after approval", async (context) => {
  ...
  const priorAgentDirectory = process.env.RIGYN_HOME;
  process.env.RIGYN_HOME = agentDirectory;
  try {
    const resolver = new ProjectTrustResolver(
    ...
    if (priorAgentDirectory === undefined) delete process.env.RIGYN_HOME;
    else process.env.RIGYN_HOME = priorAgentDirectory;
...
test("trusted runtimes discover native and compatible project skill roots only after approval", async (context) => {
  ...
  const environment: NodeJS.ProcessEnv = {
    ...process.env,
    RIGYN_HOME: join(value.root, "agent"),
...
test("CLI trust and fork flags enforce conflicts while noninteractive overrides stay invocation-only", async (context) => {
  ...
  await mkdir(join(value.workspace, ".rigyn"), { recursive: true });
  await writeFile(join(value.workspace, ".rigyn", "config.json"), "{ invalid project settings");
  const environment: NodeJS.ProcessEnv = {
    ...process.env,
...
test("an explicit one-shot TTY run does not open the project trust chooser", {
  ...
  await mkdir(join(value.workspace, ".rigyn"), { recursive: true });
  await writeFile(join(value.workspace, ".rigyn", "config.json"), "{ ignored while untrusted");
  const agentDirectory = join(value.root, "agent");
  await mkdir(agentDirectory, { recursive: true, mode: 0o700 });
  const environment: NodeJS.ProcessEnv = {
    ...process.env,
