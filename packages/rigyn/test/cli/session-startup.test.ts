...
test("startup session storage honors invocation, environment, and trusted settings precedence", async () => {
  ...
  await writeFile(join(workspace, ".rigyn", "config.json"), JSON.stringify({ sessionDir: "project-sessions" }));
  ...
  assert.equal(
    await resolveStartupSessionDirectory({}, workspace, {
      agentDirectory,
      environment: { RIGYN_SESSION_DIR: join(base, "environment-sessions") },
      projectTrusted: true,
    }),
