...
test("built-in compatible provider presets use their documented model endpoints and credentials", async () => {
  const endpoints = {
    ...
    "qwen-token-plan": "https://token-plan.ap-southeast-1.maas.aliyuncs.com/compatible-mode/v1/models",
    "qwen-token-plan-cn": "https://token-plan.cn-beijing.maas.aliyuncs.com/compatible-mode/v1/models",
...
test("Kimi and MiniMax coding presets use their Anthropic Messages model endpoints", async () => {
  const endpoints = {
    ...
    "vercel-ai-gateway": "https://ai-gateway.vercel.sh/models?limit=100",
  } as const;
  const credentials = new Map(Object.keys(endpoints).map((provider) => [
    provider,
    { kind: "api_key" as const, provider, apiKey: `fixture-${provider}` },
...
test("provider presets select only their documented compatibility profiles", () => {
  ...
  assert.equal(BUILTIN_PROVIDER_CONFIGS["vercel-ai-gateway"]?.kind, "anthropic");
...
test("Fireworks routes its maintained models through their declared protocols", async () => {
  const config = BUILTIN_PROVIDER_CONFIGS.fireworks;
  assert.equal(config?.kind, "routed");
  if (config?.kind !== "routed") throw new Error("Expected routed Fireworks provider");
  assert.equal(config.routes.length, 16);
  assert.equal(config.routes.filter((route) => route.protocolFamily === "anthropic-messages").length, 14);
  assert.equal(config.routes.filter((route) => route.protocolFamily === "openai-chat-completions").length, 2);
  assert.equal(
    config.routes.find((route) => route.model === "accounts/fireworks/models/glm-5p2")?.protocolFamily,
    "openai-chat-completions",
  );
  assert.equal(
    config.routes.find((route) => route.model === "accounts/fireworks/models/kimi-k2p6")?.protocolFamily,
    "anthropic-messages",
  );

  const broker = new CredentialBroker([new ExplicitCredentialSource(new Map([
    ["fireworks", { kind: "api_key" as const, provider: "fireworks", apiKey: "fireworks-key" }],
  ]))]);
  let networkCalls = 0;
  const adapter = createProviderAdapter(config, broker, {
    fetch: (async () => {
  ...
  assert.equal((await adapter.listModels(new AbortController().signal)).length, 16);
  assert.equal(networkCalls, 0);
});
