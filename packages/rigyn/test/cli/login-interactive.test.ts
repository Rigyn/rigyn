...
import { browserLaunch, createInteractiveAuthorizationUi, loginInteractively } from "../../src/cli/main.js";
...
function fakeLogin(input: {
  ...
  const terminal = {
    ...
    readSecret: async () => typeof input.secret === "function" ? input.secret() : input.secret ?? "fixture-secret",
...
test("interactive OAuth notification and browser launch have one owner each", async () => {
  ...
  } as unknown as Pick<TuiController, "notify" | "question">;
...
test("explicit xAI API-key login remains available without a registration warning", async () => {
  const previous = process.env.RIGYN_XAI_OAUTH_CLIENT_ID;
  delete process.env.RIGYN_XAI_OAUTH_CLIENT_ID;
  try {
    const fixture = fakeLogin({
      methods: [{ id: "api_key", kind: "api_key", label: "API key", detail: "stored" }],
      secret: "xai-test-key",
    ...
    if (previous === undefined) delete process.env.RIGYN_XAI_OAUTH_CLIENT_ID;
    else process.env.RIGYN_XAI_OAUTH_CLIENT_ID = previous;
  }
});
