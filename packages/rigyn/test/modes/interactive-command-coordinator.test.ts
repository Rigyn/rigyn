...
test("interactive coordinator dispatches every built-in and canonical alias", async () => {
  ...
  assert.equal(await coordinator.dispatchSlash("/models provider/model", []), true);
  ...
  assert.deepEqual(calls.slice(-3).map((call) => call.command), ["quit", "new", "model"]);
  assert.deepEqual(calls.at(-1), { command: "model", args: "provider/model", images: [] });
  assert.deepEqual(unknown, [removedCommand.slice(1), "extension-command"]);
});
