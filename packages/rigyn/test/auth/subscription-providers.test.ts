...
const ANTHROPIC_CLIENT_ID = "rigyn-anthropic-test-client";
const GITHUB_CLIENT_ID = "rigyn-github-test-client";
...
test("configured Anthropic OAuth uses bounded manual PKCE fallback and provider-specific refresh", async (t) => {
  ...
  assert.equal(refreshBody?.client_id, ANTHROPIC_CLIENT_ID);
...
test("GitHub Copilot login requires its own client ID and explicit experimental token-broker opt-in", async () => {
  ...
  assert.equal(serviceRequest?.headers.get("user-agent"), "rigyn/0.1.0");
  assert.equal(serviceRequest?.headers.get("editor-version"), "rigyn/0.1.0");
  assert.equal(serviceRequest?.headers.get("editor-plugin-version"), "rigyn/0.1.0");
  assert.equal(serviceRequest?.headers.get("copilot-integration-id"), "rigyn");
