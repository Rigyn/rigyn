...
test("bash rejects non-zero exits and ignores output callbacks after an operation settles", async (t) => {
  const workspace = await fixture();
  t.after(async () => await workspace.close());
  ...
  await assert.rejects(failed.execute({ command: "ignored" }, workspace.context), /Command exited with code 7/u);
});
