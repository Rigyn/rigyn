  ...
  async execute(input: JsonValue, context: ToolContext): Promise<ToolResult> {
    ...
    return await withFileMutation(absolute, async () => {
      ...
      try {
        ...
        const codeDescriptor = isError?.(error) === true
          ? Object.getOwnPropertyDescriptor(error as object, "code")
          : undefined;
        const code = codeDescriptor !== undefined && "value" in codeDescriptor && typeof codeDescriptor.value === "string"
