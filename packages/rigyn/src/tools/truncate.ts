...
export type TruncationResult = ToolTruncation;
...
function result(
  ...
  return {
    ...
    outputLines: options.outputLines ?? lines(content).length,
...
export function truncateToolHead(
  ...
  return result(selected.join("\n"), source, by, { outputLines: selected.length, maxLines, maxBytes });
}

export const truncateHead = truncateToolHead;
...
export const truncateTail = truncateToolTail;
