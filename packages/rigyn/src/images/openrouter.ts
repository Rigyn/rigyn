...
function retryable(error: unknown): boolean {
  ...
  return errorInstance(error, TypeError) || (typeof name === "string" && /connection|timeout|network/iu.test(name));
}
...
function errorHeaders(error: unknown): Headers | undefined {
  for (const current of errorChain(error)) {
    ...
    if (headers !== null && typeof headers === "object" && !isProxy(headers) && headers instanceof Headers) return headers;
  }
  return undefined;
}
...
function collectErrorText(value: unknown, output: string[], depth: number): void {
  ...
  if (typeof value === "object" && isProxy(value)) return;
