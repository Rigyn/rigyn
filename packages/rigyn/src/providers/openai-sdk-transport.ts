...
async function* sdkAzureResponsesWireEvents(
  ...
  try {
    ...
    const rigynError = nestedProviderError(error);
...
function nestedProviderError(error: unknown): HttpResponseError | ProtocolError | undefined {
  let current = error;
...
async function* safeSdkResponseEvents(
  ...
  try {
    ...
    throw new PrematureStreamEndError(
      ...
      undefined,
      { cause: translated, ...(code === undefined ? {} : { transportCode: code }) },
    );
  }
}
