...
function exactMatch(pattern: string, provider: string, model: string): boolean {
  const normalized = pattern.toLowerCase();
  return normalized === model.toLowerCase() || normalized === `${provider}/${model}`.toLowerCase();
}
...
function localScopeMatch(
  ...
  if (exactMatch(source, provider, model)) return { kind: "literal" };
  const parsed = parseModelReasoningReference(source);
  if (
    pattern.reasoningEffort !== undefined &&
    exactMatch(parsed.reference, provider, model)
  ) {
    return { kind: "thinking", reasoningEffort: pattern.reasoningEffort };
  }
...
export function modelMatchesScope(provider: string, model: string, patterns: readonly string[]): boolean {
  ...
  return patterns.some((pattern) => pattern !== SCOPED_MODELS_NONE
    && localScopeMatch(provider, model, pattern, compileScopePattern(pattern)).kind !== undefined);
}
...
export function resolveModelsForScope<T extends ScopedModelSelection>(
  ...
  for (const pattern of patterns) {
    ...
    const parsed = parseModelReasoningReference(pattern);
    const exactLiteralMatches = available.filter((model) =>
      exactMatch(pattern, model.provider, model.model));
    const exactThinkingMatches = compiled.reasoningEffort === undefined
      ? []
      : available.filter((model) => exactMatch(parsed.reference, model.provider, model.model));
    const literalMatches = available.filter((model) => matches(compiled.full, model.provider, model.model));
    const thinkingMatches = compiled.shorthand === undefined || compiled.reasoningEffort === undefined
      ? []
      : available.filter((model) => matches(compiled.shorthand!, model.provider, model.model));
    const matchesForPattern = exactLiteralMatches.length > 0
      ? exactLiteralMatches.map((model) => ({ model, matched: { kind: "literal" as const } }))
      : exactThinkingMatches.length > 0
        ? exactThinkingMatches.map((model) => ({
            model,
