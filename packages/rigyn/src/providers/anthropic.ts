...
export class AnthropicAdapter implements ProviderAdapter {
  ...
  #modelCompatibility(
    ...
    return {
      ...
      ...configuredDefault,
      ...configuredModel,
      ...(requestCompatibility?.forceAdaptiveThinking === undefined
        ? {}
        : { mode: requestCompatibility.forceAdaptiveThinking ? "adaptive" as const : "enabled" as const }),
      ...(requestCompatibility?.allowEmptySignature === undefined
        ? {}
        : { allowEmptySignature: requestCompatibility.allowEmptySignature }),
      ...(requestCompatibility?.supportsLongCacheRetention === undefined
        ? {}
        : { supportsLongCacheRetention: requestCompatibility.supportsLongCacheRetention }),
      ...(requestCompatibility?.supportsCacheControlOnTools === undefined
        ? {}
        : { supportsCacheControlOnTools: requestCompatibility.supportsCacheControlOnTools }),
      ...(requestCompatibility?.sendSessionAffinityHeaders === undefined
        ? {}
        : { sendSessionAffinityHeaders: requestCompatibility.sendSessionAffinityHeaders }),
      ...(requestCompatibility?.supportsTemperature === undefined
        ? {}
        : { supportsTemperature: requestCompatibility.supportsTemperature }),
    };
  }
    ...
    .map((message) => {
      ...
      const content = message.content.flatMap((block): unknown[] => {
        ...
        if (block.type === "tool_result") {
          ...
          const trailing = typeof content === "string" ? [{ type: "text", text: content }] : content;
          return [
...
function anthropicCacheWrite1hTokens(usage: Record<string, unknown>): unknown {
...
function normalizeAnthropicThinkingConfig(value: AnthropicThinkingConfig | undefined): AnthropicThinkingConfig | undefined {
  ...
  if (input.models !== undefined) {
    ...
    for (const [model, rawCompatibility] of Object.entries(rawModels)) {
      ...
      for (const field of [
        "supportsAdaptiveEffort", "supportsLongCacheRetention", "supportsCacheControlOnTools", "sendSessionAffinityHeaders",
        "supportsTemperature", "supportsTemperatureWithThinking",
      ] as const) {
        if (compatibility[field] !== undefined && typeof compatibility[field] !== "boolean") {
          throw new TypeError(`Anthropic thinking model ${model}.${field} must be a boolean`);
        }
      }
      models[model] = {
        ...
        ...(compatibility.supportsAdaptiveEffort === undefined
          ? {}
          : { supportsAdaptiveEffort: compatibility.supportsAdaptiveEffort as boolean }),
        ...(compatibility.supportsLongCacheRetention === undefined
          ? {}
          : { supportsLongCacheRetention: compatibility.supportsLongCacheRetention as boolean }),
        ...(compatibility.supportsCacheControlOnTools === undefined
          ? {}
          : { supportsCacheControlOnTools: compatibility.supportsCacheControlOnTools as boolean }),
        ...(compatibility.sendSessionAffinityHeaders === undefined
          ? {}
          : { sendSessionAffinityHeaders: compatibility.sendSessionAffinityHeaders as boolean }),
        ...(compatibility.supportsTemperature === undefined
          ? {}
          : { supportsTemperature: compatibility.supportsTemperature as boolean }),
        ...(compatibility.supportsTemperatureWithThinking === undefined
          ? {}
          : { supportsTemperatureWithThinking: compatibility.supportsTemperatureWithThinking as boolean }),
      };
    }
  }
...
function vercelGatewayCapabilities(
  ...
  const parameters = new Set(
    asArray(model?.supported_parameters).filter((value): value is string => typeof value === "string"),
  );
  const tags = new Set(asArray(model?.tags).filter((value): value is string => typeof value === "string"));
  const inputModalities = asArray(asRecord(model?.modalities)?.input)
...
function anthropicRequestUrl(baseUrl: string, path: string): string {
  ...
  if (/\/messages$/u.test(root)) root = root.slice(0, -"/messages".length);
  if (!/\/v1$/u.test(root)) root += "/v1";
