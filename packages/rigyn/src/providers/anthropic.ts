...
function buildAnthropicBody(
  ...
  if (request.temperature !== undefined) {
    if (!Number.isFinite(request.temperature) || request.temperature < 0 || request.temperature > 1) {
      throw new InvalidProviderRequestError("Anthropic temperature must be between 0 and 1");
    }
    const thinkingEnabled = request.reasoningEffort !== undefined &&
      request.reasoningEffort !== "off" && request.reasoningEffort !== "none";
    if (!thinkingEnabled && compatibility.supportsTemperature) body.temperature = request.temperature;
  }
  ...
  if (input.models !== undefined) {
    ...
    for (const [model, rawCompatibility] of Object.entries(rawModels)) {
      ...
      assertObjectKeys(compatibility, [
        "mode", "off", "interleaved", "allowEmptySignature", "supportsLongCacheRetention",
        "supportsCacheControlOnTools", "sendSessionAffinityHeaders", "supportsTemperature",
      ...
      for (const field of [
        "supportsLongCacheRetention", "supportsCacheControlOnTools", "sendSessionAffinityHeaders", "supportsTemperature",
      ] as const) {
