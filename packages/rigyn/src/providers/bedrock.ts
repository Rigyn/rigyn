...
export class BedrockAdapter implements ProviderAdapter {
  ...
  async #signedFetch(
    ...
    region: string,
    signal: AbortSignal,
    inspectRequest?: (body: Record<string, unknown>) => void | Promise<void>,
  ): Promise<Response> {
...
async function readRequestJson(request: Request): Promise<{ request: Request; body: Record<string, unknown> }> {
  const buffered = await bufferRequestBody(request, "Bedrock request body");
  const text = new TextDecoder().decode(buffered.body);
  const parsed: unknown = JSON.parse(text);
...
function bedrockReasoningVisibility(body: Record<string, unknown>): "summary" | "provider_trace" {
