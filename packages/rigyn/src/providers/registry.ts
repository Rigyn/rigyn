...
function isCatalogValidationError(error: unknown): error is CatalogValidationError {
  const isError = (Error as ErrorConstructor & { isError?: (candidate: unknown) => boolean }).isError;
  return isError?.(error) === true && error instanceof CatalogValidationError;
}
...
function configuredReasoningEffortMap(
  ...
  for (const [effort, raw] of entries) {
    ...
    result[effort as ModelReasoningEffort] = mapped;
  }
  return result;
}
...
function configuredRequestCompatibility(value: unknown, label: string): ModelRequestCompatibility | undefined {
  ...
  return {
    ...
    ...(supportsToolReferences === undefined ? {} : { supportsToolReferences }),
    ...
    ...(supportsOpenAIGrammarTools === undefined ? {} : { supportsOpenAIGrammarTools }),
    ...(supportsStrictTools === undefined ? {} : { supportsStrictTools }),
    ...(maxTokensField === undefined ? {} : { maxTokensField }),
    ...(requiresToolResultName === undefined ? {} : { requiresToolResultName }),
    ...(requiresAssistantAfterToolResult === undefined ? {} : { requiresAssistantAfterToolResult }),
    ...
    ...(exposesReasoningText === undefined ? {} : { exposesReasoningText }),
    ...(supportsThinkingDisplay === undefined ? {} : { supportsThinkingDisplay }),
    ...(reasoningOutputFormat === undefined ? {} : { reasoningOutputFormat }),
    ...
    ...(supportsExplicitPromptCacheMode === undefined ? {} : { supportsExplicitPromptCacheMode }),
    ...(supportsPromptCacheBreakpoints === undefined ? {} : { supportsPromptCacheBreakpoints }),
...
export function parseStoredProviderModels(
  providerValue: unknown,
  value: unknown,
  label = "models",
): ProviderModel[] {
  const provider = configuredId(providerValue, MAX_PROVIDER_ID_BYTES, "provider") as ProviderId;
  ...
  return value.map((entry, index): ProviderModel => {
    ...
    seen.add(id);
    const modelProvider = configuredId(input.provider, MAX_PROVIDER_ID_BYTES, `${modelLabel}.provider`) as ProviderId;
    ...
    if (input.cost === null || typeof input.cost !== "object" || Array.isArray(input.cost)) {
      throw new CatalogValidationError(`${modelLabel}.cost must be an object`);
    }
    const costInput = input.cost as Record<string, unknown>;
    ...
    if (costInput.tiers !== undefined) {
      ...
      tiers = costInput.tiers.map((tier, tierIndex) => {
        ...
        const tierInput = tier as Record<string, unknown>;
      ...
      if (new Set(tiers.map((tier) => tier.inputTokensAbove)).size !== tiers.length) {
    ...
    return {
      ...
      input: inputModes,
      cost: { ...cost, ...(tiers === undefined ? {} : { tiers }) },
      contextWindow,
...
function applyConfiguredModel(
  ...
  return {
    ...
    ...(maintainedFallback(base.maxInputTokens, configuration.maxInputTokens) === undefined
      ? {}
      : { maxInputTokens: maintainedFallback(base.maxInputTokens, configuration.maxInputTokens)! }),
...
function configuredModelRequestSettings(configuration: ConfiguredModel | undefined): ProviderModelRequestSettings | undefined {
  ...
  return {
    ...(configuration.displayName === undefined ? {} : { displayName: configuration.displayName }),
