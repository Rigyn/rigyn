...
function geminiThinkingConfig(request: ProviderRequest): Record<string, unknown> | undefined {
  ...
  if (/^gemini-2\.5(?:-|$)/u.test(model)) {
    ...
    const configured = request.thinkingBudgets?.[
      requested as keyof NonNullable<ProviderRequest["thinkingBudgets"]>
    ];
    return {
    ...
    .map((message) => {
      ...
      const parts = message.content.flatMap((block): unknown[] => {
        ...
        if (block.type === "tool_call") {
          return [{
            functionCall: {
              ...(includeToolCallId ? { id: block.callId } : {}),
              name: block.name,
...
function supportsVertexPublisherGenerateContent(model: string): boolean {
  return /^gemini-/u.test(model) && !/(?:^|-)embedding(?:-|$)/u.test(model);
}
