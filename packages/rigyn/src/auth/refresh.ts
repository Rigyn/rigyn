    ...
    return this.#store.withLock(
      ...
      async () => {
        ...
        const next: OAuthCredential = {
          ...
          ...((current.tokenEndpoint === undefined && refreshed.tokenEndpoint === undefined)
            ? {}
            : { tokenEndpoint: refreshed.tokenEndpoint ?? current.tokenEndpoint }),
          ...(current.revocationEndpoint === undefined ? {} : { revocationEndpoint: current.revocationEndpoint }),
          ...((current.clientId === undefined && refreshed.clientId === undefined)
            ? {}
            : { clientId: refreshed.clientId ?? current.clientId }),
