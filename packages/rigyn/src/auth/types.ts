...
export function isMutableCredentialStore(store: CredentialStore): store is MutableCredentialStore {
  const candidate = store as Partial<MutableCredentialStore>;
  return typeof candidate.list === "function" && typeof candidate.modify === "function";
}
...
function boundedSecretText(value: unknown, maximum: number, optional = true): boolean {
  return boundedText(value, maximum, optional) &&
    (value === undefined || (value as string).length >= MIN_REDACTABLE_SECRET_CHARACTERS);
}
...
function credentialEnvironment(value: unknown): value is Record<string, string> {
