...
const ENABLE_MOUSE = "\u001b[?1000h\u001b[?1002h\u001b[?1006h\u001b[?1003h\u001b[?1004h";
const DISABLE_MOUSE = "\u001b[?1004l\u001b[?1003l\u001b[?1006l\u001b[?1002l\u001b[?1000l";
...
const MAX_CLASSIC_EVENT_PREVIEW_BYTES = 8 * 1024;
...
function terminalIdentityTitle(context: TuiContext): string {
  ...
  return byteTruncate(["rigyn", ...parts.filter((part, index) => parts.indexOf(part) === index)].join(" · "), MAX_TERMINAL_TITLE_BYTES);
}
...
function tmuxModifiedEnterWarning(options: TmuxKeyOptions): string | undefined {
  ...
  return `rigyn detected tmux extended-keys=off${formatDetail}. Modified Enter shortcuts may be read as plain Enter. Set extended-keys to on in tmux, then reattach the client.`;
}
...
function sameQueuedMessageShape(left: QueuedMessage, right: QueuedMessage): boolean {
  return left.mode === right.mode
    && queuedMessageImageCount(left) === queuedMessageImageCount(right);
}
...
interface Overlay {
  ...
  scopedModels?: {
    ...
    order: string[];
    all: boolean;
    committed: ScopedModelSelection;
    live?: boolean;
...
function cancellationError(value: unknown, fallback: string): Error {
  return isErrorObject(value) ? value : new Error(fallback);
}
...
export class TuiController {
  ...
  readonly #classicReasoningParts = new Map<string, string>();
  readonly #classicPendingText = new Map<string, { chunks: string[]; bytes: number }>();
  readonly #classicTextStarted = new Set<string>();
  readonly #classicToolArgumentParts = new Map<string, { bytes: number; truncated: boolean }>();
  ...
  #transcriptNavigationSize: { columns: number; rows: number } | undefined;
  #transcriptViewportAnchor: NonNullable<ReturnType<typeof internalTranscriptFrameAnchorState>>["viewport"];
    ...
    try {
      ...
      if (this.#alternateInput !== undefined) {
        ...
        for (const event of parsed.mouse) {
          ...
          this.#handleAlternateDecision(this.#alternateInteraction!.handle(event));
        }
        if (parsed.focusLost) {
          this.#handleAlternateDecision(this.#alternateInteraction!.cancelPointer());
        }
        if (parsed.mouse.length > 0 || parsed.focusLost) this.#scheduleRender();
  ...
  setOperatorPreferences(preferences: Partial<TuiOperatorPreferences>): void {
    ...
    if (preferences.showTerminalProgress !== undefined) {
      if (typeof preferences.showTerminalProgress !== "boolean") throw new TypeError("showTerminalProgress must be boolean");
    ...
    if (preferences.fullscreenScrollbar !== undefined) {
      if (!["auto", "always", "never"].includes(preferences.fullscreenScrollbar)) {
        throw new Error("fullscreenScrollbar is invalid");
      }
      this.#fullscreenScrollbar = preferences.fullscreenScrollbar === "never"
        ? "hidden"
        : preferences.fullscreenScrollbar;
    }
    for (const owner of [...this.#rawEditors]) {
  ...
  installRawEditor(
    ...
    const owner: RawEditorOwner = {
      ...
      ...(updateAutocomplete === undefined ? {} : { updateAutocomplete }),
      onAbort: () => undefined,
    };
    ...
    const previous = previousOwner?.component as (EditorComponent & { focused?: boolean }) | undefined;
    if (previous !== undefined && "focused" in previous) previous.focused = false;
    const focusable = component as EditorComponent & { focused?: boolean };
    if ("focused" in focusable) focusable.focused = true;
    let disposed = false;
    const dispose = (): void => {
      if (disposed) return;
      disposed = true;
      ...
      if (text !== undefined) {
        ...
        if (successor === undefined) this.#editor.setText(text);
          ...
          const successorFocusable = successor as EditorComponent & { focused?: boolean };
          if ("focused" in successorFocusable) successorFocusable.focused = true;
        }
      }
      if ("focused" in focusable) focusable.focused = false;
  ...
  rawEditorPreferences(): { paddingX: number; autocompleteMaxVisible: number } {
    return {
    ...
    for (const batch of batches) {
      ...
      for (const update of updates) {
        ...
        this.#model.applyToolProgressOutputBatch(output.envelope, {
          ...
          ...(output.elapsedMs === undefined ? {} : { elapsedMs: output.elapsedMs }),
  ...
  replaceTranscript(
    ...
    if (this.mode === "full") this.#scheduleRender();
      ...
      const rendered = renderTranscriptFrame(entries, size.columns, this.#theme, {
        ...
        ...(transformMarkdown === undefined ? {} : { transformMarkdown }),
      });
  ...
  chooseSettings(
    ...
    if (this.mode !== "full") {
      return this.#chooseSettingsByLine(source.map((item) => item.value), onChange, signal);
    }
  ...
  async #chooseScopedModelsByLine(
    models: readonly PickerItem<ScopedModelOption>[],
    options: {
      all: boolean;
    ...
    const initial: ScopedModelSelection = options.all
      ? { mode: "all" }
      : options.selected.length === 0 ? { mode: "none" } : { mode: "models", patterns: [...options.selected] };
    let all = options.all;
    const selected = new Set(options.selected);
    let order = [...options.selected];
    const selection = (): ScopedModelSelection => all
      ? { mode: "all" }
      : order.length === 0 ? { mode: "none" } : { mode: "models", patterns: [...order] };
    while (true) {
      let value: string;
      try {
        value = await this.#chooseByLine("Scoped Models", [
          { label: "Save selection", detail: "Apply and close", value: SCOPED_MODEL_SAVE },
          { label: "Enable all models", detail: "Include current and future models", value: SCOPED_MODEL_ALL },
          { label: "Clear all models", detail: "Disable model cycling", value: SCOPED_MODEL_NONE },
          ...models.map((item) => {
            const pattern = this.#scopedModelPattern(item)!;
            return {
              label: `${all || selected.has(pattern) ? "[x]" : "[ ]"} ${item.label}`,
              detail: item.detail ?? pattern,
              value: pattern,
            };
          }),
        ], signal);
      } catch (cause) {
        if (cause instanceof TuiSelectionCancelledError) {
          options.onChange?.(initial);
          return initial;
        }
        throw cause;
      }
      if (value === SCOPED_MODEL_SAVE) {
        const result = selection();
        await options.onSave?.(result);
        return result;
      }
      if (value === SCOPED_MODEL_ALL) {
        all = true;
        selected.clear();
        order = [];
      } else if (value === SCOPED_MODEL_NONE) {
        all = false;
        selected.clear();
        order = [];
      } else {
        if (all) {
          ...
          order = order.filter((pattern) => pattern !== value);
        } else {
          selected.add(value);
          order.push(value);
        }
  ...
  chooseScopedModels(
    ...
    const configured = [...new Set(options.selected)].filter((pattern) =>
      pattern !== "!none" &&
      pattern.length > 0 &&
      Buffer.byteLength(pattern, "utf8") <= 256 &&
      !/[\u0000-\u001f\u007f]/u.test(pattern)).slice(0, 100);
    ...
    const unavailable = configured.filter((pattern) => !seen.has(pattern)).map((pattern, index) => {
      const separator = pattern.indexOf("/");
      return {
        id: `configured-model:${index}:${pattern}`,
        label: `${pattern} · unavailable`,
        detail: "Configured · currently unavailable",
        keywords: ["configured unavailable", pattern],
        value: {
          provider: separator > 0 ? pattern.slice(0, separator) : "configured",
          model: separator > 0 ? pattern.slice(separator + 1) : pattern,
          pattern,
        },
      } satisfies PickerItem<ScopedModelOption>;
    }).slice(0, maximumModels);
    const availableByPattern = new Map(available.map((item) => [
      `${item.value.provider}/${item.value.model}`,
      item,
    ]));
    const selectedAvailable = configured.flatMap((pattern) => {
      const item = availableByPattern.get(pattern);
      if (item === undefined) return [];
      availableByPattern.delete(pattern);
      return [item];
    });
    const models = [
      ...selectedAvailable,
      ...unavailable,
      ...available.filter((item) => availableByPattern.has(`${item.value.provider}/${item.value.model}`)),
    ].slice(0, maximumModels);
    if (this.mode !== "full") {
      return this.#chooseScopedModelsByLine(models, {
        all: options.all,
        selected: configured,
    ...
    return new Promise<ScopedModelSelection>((resolve, reject) => {
      ...
      this.#overlay.scopedModels = {
        source: models,
        selected: new Set(configured.filter((pattern) =>
          models.some((item) => this.#scopedModelPattern(item) === pattern))),
        order: configured.filter((pattern) =>
          models.some((item) => this.#scopedModelPattern(item) === pattern)),
        all: options.all,
        committed: options.all
          ? { mode: "all" }
          : configured.length === 0 ? { mode: "none" } : { mode: "models", patterns: [...configured] },
  ...
  chooseSessionTree<T>(
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      overlay.tree = {
        ...
        ...(preferredActive?.tree === undefined ? {} : { preferredActiveEventId: preferredActive.tree.eventId }),
        ...(preferred?.tree === undefined ? {} : { selectedEventId: preferred.tree.eventId }),
      };
      this.#refreshOverlay();
      this.#scheduleRender();
    });
  ...
  setQueuedMessages(messages: readonly QueuedMessage[]): void {
    ...
    for (const addition of additions) {
      ...
      const selected = exact ?? this.#pendingActiveMessages.find((pending) =>
        pending.canonical === undefined && sameQueuedMessageShape(pending.display, addition));
    ...
    this.#pendingActiveMessages = this.#pendingActiveMessages.filter((pending) => {
      ...
      if (index < 0) {
        index = next.findIndex((candidate, candidateIndex) =>
          !claimed.has(candidateIndex) && sameQueuedMessageShape(candidate, canonical));
      }
  ...
  #observeActiveDelivery(delivery: unknown, pending?: PendingActiveMessage): void {
    ...
    if (
      ...
      || typeof (delivery as PromiseLike<void>).then !== "function"
    ) return;
    void Promise.resolve(delivery as PromiseLike<void>).catch((cause: unknown) => {
  ...
  setModelPickerItems<T>(all: readonly PickerItem<T>[], scoped?: readonly PickerItem<T>[]): void {
    ...
    if (this.#overlay?.kind === "model") {
      const hadItems = this.#overlay.items.length > 0;
      ...
      this.#refreshOverlay();
      if (!hadItems && this.#overlay.query.empty) this.#selectCurrentModelRow();
      this.#scheduleRender();
    }
  }
  ...
  footerDataSnapshot(): Readonly<FooterDataSnapshot> {
    ...
    return Object.freeze({
      ...
      ...(context.sessionName === undefined ? {} : { sessionName: context.sessionName }),
      ...(context.releaseVersion === undefined ? {} : { releaseVersion: context.releaseVersion }),
      ...(context.active === undefined ? {} : { active: context.active }),
      ...(context.status === undefined ? {} : { status: context.status }),
      ...(context.activity === undefined
        ? {}
        : {
      ...
      ...(workingIndicator === undefined
        ...
        : {
            workingIndicator: Object.freeze({
              ...
              ...(workingIndicator.hidden === undefined ? {} : { hidden: workingIndicator.hidden }),
            }),
          }),
      ...(context.provider === undefined ? {} : { provider: context.provider }),
      ...(context.model === undefined ? {} : { model: context.model }),
      ...(context.thinking === undefined ? {} : { thinking: context.thinking }),
      ...(context.thinkingSupported === undefined ? {} : { thinkingSupported: context.thinkingSupported }),
      ...(usage?.total.inputTokens === undefined ? {} : { inputTokens: usage.total.inputTokens }),
      ...(usage?.total.outputTokens === undefined ? {} : { outputTokens: usage.total.outputTokens }),
      ...(usage?.promptInputTokens === undefined ? {} : { promptInputTokens: usage.promptInputTokens }),
      ...(usage?.reportedPromptInputTokens === undefined
        ? {}
        : { promptInputTokensReported: usage.reportedPromptInputTokens }),
      ...(usage?.total.inputTokens !== undefined || usage?.reportedTotal?.inputTokens === undefined
        ? {}
        : { inputTokensReported: usage.reportedTotal.inputTokens }),
      ...(usage?.total.outputTokens !== undefined || usage?.reportedTotal?.outputTokens === undefined
        ? {}
        : { outputTokensReported: usage.reportedTotal.outputTokens }),
      ...(usage?.total.cacheReadTokens === undefined ? {} : { cacheReadTokens: usage.total.cacheReadTokens }),
      ...(usage?.total.cacheWriteTokens === undefined ? {} : { cacheWriteTokens: usage.total.cacheWriteTokens }),
      ...(usage?.total.cacheWrite1hTokens === undefined ? {} : { cacheWrite1hTokens: usage.total.cacheWrite1hTokens }),
      ...(usage?.latestCacheReadTokens === undefined ? {} : { latestCacheReadTokens: usage.latestCacheReadTokens }),
      ...(usage?.latestCacheWriteTokens === undefined ? {} : { latestCacheWriteTokens: usage.latestCacheWriteTokens }),
      ...(usage?.latestCacheWrite1hTokens === undefined
        ? {}
        : { latestCacheWrite1hTokens: usage.latestCacheWrite1hTokens }),
      ...(usage?.latestCacheHitRate === undefined ? {} : { cacheHitRate: usage.latestCacheHitRate }),
      ...(usage?.total.cost?.total === undefined ? {} : { cost: usage.total.cost.total }),
      ...
      ...(context.contextSource === undefined ? {} : { contextSource: context.contextSource }),
      ...(context.autoCompaction === undefined ? {} : { autoCompaction: context.autoCompaction }),
      ...(context.autoCompactionThresholdPercent === undefined
        ? {}
        : { autoCompactionThresholdPercent: context.autoCompactionThresholdPercent }),
      ...(context.subscription === undefined ? {} : { subscription: context.subscription }),
    });
  }
  ...
  #renderToolBlocks(entries: readonly TranscriptEntry[], width: number, height: number): Map<string, ToolRenderSlots> {
    ...
    for (const entry of entries) {
      ...
      const view = immutableRuntimeToolRenderView({
        ...
        ...(entry.toolData?.progress === undefined ? {} : { progress: entry.toolData.progress }),
  ...
  #keyHintRenderOptions(includeThinking = true): { expandKeyHint: string | undefined; thinkingKeyHint?: string } {
    ...
    return {
      expandKeyHint,
      ...(!includeThinking || thinkingKeyHint === undefined ? {} : { thinkingKeyHint }),
    };
  }
  ...
  #renderPersistentComponents(size: { columns: number; rows: number }): Record<TuiPersistentComponentSlot, RuntimeUiBlock[]> {
    ...
    for (const slot of PERSISTENT_COMPONENT_SLOTS) {
      for (const owner of this.#persistentRuntimeComponents[slot].values()) {
        ...
        }, { maxLines: MAX_ADVANCED_UI_SOURCE_LINES, maxBytes: MAX_ADVANCED_UI_SOURCE_BYTES });
        if (rendered.ok) blocks[slot].push(truncatePersistentBlock(rendered.block, height));
  ...
  renderNow(): void {
        ...
        ? {
            ...
            emptyMessage: overlay.source.length === 0
              ? this.#modelPickerLoading
                ...
                : overlay.modelPicker?.mode === "scoped" && overlay.modelPicker.all.length > 0
                  ? "No models in the current scope. Press Tab to show all models."
    ...
    const view: TuiViewState = {
      ...
      ...(this.#pendingQuestion === undefined ? {} : { inputPrompt: this.#pendingQuestion.prompt }),
    ...
    const renderOptions = {
      ...
      ...(transformMarkdown === undefined ? {} : { transformMarkdown }),
    } satisfies NonNullable<Parameters<typeof renderFrame>[3]>;
    const previousNavigation = this.#transcriptNavigation;
    const previousNavigationSize = this.#transcriptNavigationSize;
    const previousViewportAnchor = this.#transcriptViewportAnchor;
    ...
    let frame = renderFrame(view, size, this.#theme, renderOptions);
    let nextNavigation = frame.transcriptNavigation;
    let nextAnchorState = internalTranscriptFrameAnchorState(frame);
    if (
      previousNavigation !== undefined
      && previousNavigationSize?.columns === size.columns
      && previousNavigationSize.rows === size.rows
      ...
      const anchoredRow = previousViewportAnchor === undefined
        ? -1
        : nextAnchorState?.rows.findIndex((candidate) =>
            candidate?.entryId === previousViewportAnchor.anchor.entryId
            && candidate.renderedRow === previousViewportAnchor.anchor.renderedRow) ?? -1;
      ...
      const anchoredStart = Math.max(
        ...
        Math.min(
          ...
          anchoredRow < 0
            ? previousNavigation.startRow
            : anchoredRow - previousViewportAnchor!.row,
        ),
      );
      ...
      if (anchoredOffset !== this.#transcriptOffset) {
        ...
        frame = renderFrame({ ...view, transcriptOffset: anchoredOffset }, size, this.#theme, renderOptions);
        nextNavigation = frame.transcriptNavigation;
        nextAnchorState = internalTranscriptFrameAnchorState(frame);
      }
    }
    this.#transcriptNavigation = nextNavigation;
    this.#transcriptNavigationSize = { columns: size.columns, rows: size.rows };
    this.#transcriptViewportAnchor = nextAnchorState?.viewport;
    ...
    this.#alternateInteraction?.updateFrame(
      ...
      pointerRegion === undefined ? undefined : {
        ...
        ...(pointerRegion.scrollbar === undefined ? {} : {
          scrollbar: {
  ...
  close(): void {
    ...
    this.#editorRenderer = undefined;
    this.#classicReasoningParts.clear();
    this.#classicPendingText.clear();
    this.#classicTextStarted.clear();
    this.#classicToolArgumentParts.clear();
    this.#sessionEntries.clear();
    this.#sessionEntryBytes = 0;
  ...
  #handleAlternateDecision(decisions: readonly AlternateScreenDecision[]): void {
    for (const decision of decisions) {
      ...
      void this.copyToClipboard(decision.text).then(() => {
        if (!this.#closed) {
          if (this.#selectionGeneration === selectionGeneration) {
            this.#handleAlternateDecision(this.#alternateInteraction?.cancelPointer() ?? []);
          }
          this.#showCopyToast(decision.truncated ? "Copied 75 KB" : "Copied");
        }
      }, (cause: unknown) => {
        if (!this.#closed) this.notify(boundedTuiFailureText(cause), "warning");
      });
      this.#scheduleRender();
    }
  }
  ...
  #handleRawEditorHostEvent(owner: RawEditorOwner, event: KeyEvent): boolean {
    ...
    const component = owner.component as EditorComponent & { isShowingAutocomplete?(): boolean };
    if (action === "app.interrupt" && event.key === "escape" && component.isShowingAutocomplete?.() === true) {
      return false;
    }
    if (
      action === "app.exit"
  ...
  #handleOverlayKey(event: KeyEvent): void {
    ...
    if (overlay.modelPicker?.scoped !== undefined && !event.ctrl && !event.alt && !event.shift && event.key === "tab") {
      ...
      overlay.selected = 0;
      this.#refreshOverlay();
      if (overlay.query.empty) this.#selectCurrentModelRow();
      return;
    }
  ...
  #handleScopedModelOverlayKey(overlay: Overlay, event: KeyEvent): boolean {
    ...
    if (this.#keybindings.matches("app.models.enableAll", event)) {
      const filtered = overlay.query.empty ? undefined : this.#scopedModelPatterns(overlay.items);
      if (filtered === undefined) {
        ...
        scoped.status = "All matching models are already enabled";
      } else {
        for (const pattern of filtered) {
          if (scoped.selected.has(pattern)) continue;
          scoped.selected.add(pattern);
          scoped.order.push(pattern);
        }
        this.#collapseCompleteScopedSelection(scoped);
        scoped.status = `${filtered.length} matching model${filtered.length === 1 ? "" : "s"} enabled`;
      }
      this.#scopedModelsChanged(scoped);
      this.#refreshOverlay();
      return true;
    ...
    if (this.#keybindings.matches("app.models.clearAll", event)) {
      ...
      if (filtered === undefined) {
        ...
        for (const pattern of targets) scoped.selected.delete(pattern);
        scoped.order = scoped.order.filter((pattern) => scoped.selected.has(pattern));
        scoped.status = `${filtered.length} matching model${filtered.length === 1 ? "" : "s"} cleared`;
      }
      this.#scopedModelsChanged(scoped);
      this.#refreshOverlay();
      return true;
    ...
    if (this.#keybindings.matches("app.models.toggleProvider", event)) {
      ...
      const patterns = scoped.source
        ...
        .flatMap((candidate) => {
          const pattern = this.#scopedModelPattern(candidate);
    ...
    if (item.id === SCOPED_MODEL_ALL) {
      ...
      if (scoped.all) {
        scoped.all = false;
        scoped.selected = new Set([pattern]);
        scoped.order = [pattern];
      } else if (scoped.selected.has(pattern)) {
  ...
  #materializeScopedModels(scoped: NonNullable<Overlay["scopedModels"]>): void {
    scoped.all = false;
    scoped.order = scoped.source.flatMap((item) => {
      const pattern = this.#scopedModelPattern(item);
      return pattern === undefined ? [] : [pattern];
    });
  ...
  #scopedModelPatterns(items: readonly PickerItem[]): string[] {
  ...
  #collapseCompleteScopedSelection(scoped: NonNullable<Overlay["scopedModels"]>): void {
    const available = this.#scopedModelPatterns(scoped.source);
    if (available.length === 0 || available.some((pattern) => !scoped.selected.has(pattern))) return;
    scoped.all = true;
    scoped.selected.clear();
    scoped.order = [];
  }
  ...
  #scopedModelPattern(item: PickerItem): string | undefined {
    ...
    if ("pattern" in value && typeof value.pattern === "string") return value.pattern;
        ...
        if (pending !== undefined && typeof (pending as PromiseLike<void>).then === "function") {
          ...
          void Promise.resolve(pending).then(() => {
            ...
            if (this.#overlay === overlay) {
              ...
              try {
                scoped.onChange?.(selection);
              } catch {
                // The picker is already closed; the saved selection remains authoritative.
              }
            }
          }, (cause: unknown) => {
            scoped.status = boundedTuiDiagnosticText(
              `Could not save model cycling defaults: ${boundedTuiFailureText(cause)}`,
            );
            if (this.#overlay === overlay) {
              this.#refreshOverlay();
              this.#scheduleRender();
  ...
  #scopedModelsChanged(scoped: NonNullable<Overlay["scopedModels"]>): void {
    try {
      ...
      scoped.status = boundedTuiFailureText(cause);
    }
  }
  ...
  #handleEditorKey(event: KeyEvent): void {
    ...
    if (this.#inputBlocked !== undefined) {
      if (this.#keybindings.matches("app.tools.expand", event)) this.toggleTool();
    ...
    if (this.#keybindings.matches("app.interrupt", event)) {
      if (this.#steering !== undefined || this.#model.context.active === true) {
        if (this.#steering !== undefined) {
          const delivery = this.#steering("/cancel") as unknown;
  ...
  #submit(): void {
    ...
    if (this.#inputMode === "follow_up") {
      ...
      if (this.#steering !== undefined) {
        ...
        try {
          const delivery = this.#steering(`/follow ${text}`, images, recoveredImages, recoveredQueueDraft) as unknown;
    ...
    if (this.#steering !== undefined) {
      ...
      try {
        const delivery = this.#steering(text, images, recoveredImages, recoveredQueueDraft) as unknown;
        this.#observeActiveDelivery(delivery, pending);
      } catch (cause) {
        if (pending !== undefined) this.#removePendingActiveMessage(pending);
        throw cause;
  ...
  #refreshOverlay(): void {
    ...
    if (overlay.scopedModels !== undefined) {
      ...
      const byPattern = new Map(overlay.scopedModels.source.flatMap((item) => {
        const pattern = this.#scopedModelPattern(item);
        return pattern === undefined ? [] : [[pattern, item] as const];
      }));
      ...
      orderedModels.push(...overlay.scopedModels.source.filter((item) => {
        const pattern = this.#scopedModelPattern(item);
        return pattern !== undefined && byPattern.has(pattern);
      }));
  ...
  #closeOverlay(cause: Error): void {
    ...
    if (scoped?.live === true) {
      try {
        scoped.onChange?.(scoped.committed);
      } catch {
        // Cancellation still closes the picker when a preview rollback fails.
      }
    }
    this.#overlay = undefined;
    overlay.cleanup();
  ...
  #classicEventPreview(value: string): string {
    return byteTruncate(
      sanitizeTerminalText(value),
      Math.min(this.#limits.maxToolPreviewBytes, MAX_CLASSIC_EVENT_PREVIEW_BYTES),
    );
  }

  #classicReasoningKey(scope: string, part: number): string {
    return `${scope}:${part}`;
  }

  #classicHasPendingReasoning(scope: string): boolean {
    const prefix = `${scope}:`;
    return [...this.#classicReasoningParts.keys()].some((key) => key.startsWith(prefix));
  }

  #clearClassicReasoning(scope: string): void {
    const prefix = `${scope}:`;
    for (const key of this.#classicReasoningParts.keys()) {
      if (key.startsWith(prefix)) this.#classicReasoningParts.delete(key);
    }
  }

  #flushClassicPendingText(scope: string): void {
    const pending = this.#classicPendingText.get(scope);
    if (pending === undefined) return;
    this.#classicPendingText.delete(scope);
    if (pending.chunks.length === 0) return;
    this.#classicTextStarted.add(scope);
    for (const chunk of pending.chunks) this.#write(chunk);
  }

  #queueClassicText(scope: string, value: string): void {
    if (value === "") return;
    const pending = this.#classicPendingText.get(scope) ?? { chunks: [], bytes: 0 };
    pending.chunks.push(value);
    pending.bytes += Buffer.byteLength(value, "utf8");
    this.#classicPendingText.set(scope, pending);
    if (pending.bytes >= this.#limits.maxTranscriptBytes) this.#flushClassicPendingText(scope);
  }

  #resetClassicScope(scope: string, flushText: boolean): void {
    if (flushText) this.#flushClassicPendingText(scope);
    else this.#classicPendingText.delete(scope);
    this.#clearClassicReasoning(scope);
    this.#classicTextStarted.delete(scope);
  }
  ...
  #renderClassic(envelope: EventEnvelope): void {
    ...
    if (event.type === "run_started") {
    ...
    else if (event.type === "assistant_started") this.#resetClassicScope(scope, true);
    else if (event.type === "text_delta") {
      const text = sanitizeTerminalText(event.text);
      if (this.#classicTextStarted.has(scope) || !this.#classicHasPendingReasoning(scope)) {
        if (text !== "") this.#classicTextStarted.add(scope);
        this.#write(text);
      } else this.#queueClassicText(scope, text);
    }
    else if (event.type === "reasoning_started" && event.visibility === "summary") {
      const key = this.#classicReasoningKey(scope, event.part);
      if (!this.#classicReasoningParts.has(key)) this.#classicReasoningParts.set(key, "");
    }
    else if (
      ...
      const key = this.#classicReasoningKey(scope, event.part);
      const current = this.#classicReasoningParts.get(key) ?? "";
      this.#classicReasoningParts.set(key, this.#classicEventPreview(`${current}${event.text}`));
    } else if (event.type === "reasoning_completed" && event.visibility === "summary") {
      const key = this.#classicReasoningKey(scope, event.part);
      const buffered = this.#classicReasoningParts.get(key) ?? "";
      this.#classicReasoningParts.delete(key);
      if (event.redacted !== true && !this.#classicTextStarted.has(scope)) {
        const text = event.text === "" ? buffered : this.#classicEventPreview(event.text);
        if (text !== "") {
      ...
      if (!this.#classicHasPendingReasoning(scope)) this.#flushClassicPendingText(scope);
    } else if (event.type === "tool_call_started") {
      this.#write(`\n[tool planning] ${sanitizeTerminalText(event.name ?? "tool")} (call ${event.index + 1})\n`);
    } else if (event.type === "tool_call_delta") {
      const key = `${scope}:${event.index}`;
      let state = this.#classicToolArgumentParts.get(key);
      if (state === undefined) {
        state = { bytes: 0, truncated: false };
        this.#classicToolArgumentParts.set(key, state);
        this.#write(`[tool input ${event.index + 1}] `);
      }
      if (!state.truncated) {
        const safe = sanitizeTerminalText(event.jsonFragment);
        const maximum = Math.min(this.#limits.maxToolPreviewBytes, MAX_CLASSIC_EVENT_PREVIEW_BYTES);
      ...
      const key = `${scope}:${event.index}`;
      if (!this.#classicToolArgumentParts.delete(key) && event.rawArguments !== "") {
        this.#write(`[tool input ${event.index + 1}] ${this.#classicEventPreview(event.rawArguments)}`);
      }
      this.#write("\n");
      if (event.parseError !== undefined) {
        this.#write(`[tool input failed] ${sanitizeTerminalText(event.name)}: ${this.#classicEventPreview(event.parseError)}\n`);
      }
    }
      ...
      if (event.progress.type === "output") {
        if (event.progress.delta !== "") {
          const preview = this.#classicEventPreview(event.progress.delta);
        ...
        const preview = this.#classicEventPreview(event.progress.content);
      ...
      this.#resetClassicScope(scope, false);
      for (const key of this.#classicToolArgumentParts.keys()) {
        if (key.startsWith(`${scope}:`)) this.#classicToolArgumentParts.delete(key);
      }
