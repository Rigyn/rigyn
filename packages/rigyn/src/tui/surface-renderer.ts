import { isImageLine } from "@rigyn/terminal";
...
function rawKittyImageIds(rows: readonly string[]): number[] {
  ...
  for (const row of rows) {
    ...
    for (const match of row.matchAll(/\u001b_G([^;]*);/gu)) {
...
export class LiveSurfaceRenderer {
  ...
  render(frame: Frame, size: { columns: number; rows: number }): SurfaceRenderResult {
    ...
    const imageRedraw = (
      ...
      || nextImages.length > 0
      || rawImageRows
    ) && (resized || textChanged || imagesChanged);
    ...
    if (["initial", "viewport-clear", "surface-clear", "image-redraw"].includes(strategy)) {
      this.#fullRedraws += 1;
      try {
        this.#onDiagnostic?.({
          strategy: strategy as SurfaceRenderDiagnostic["strategy"],
          previousRows,
