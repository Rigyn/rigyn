...
function normalizedRuntimeColor(value: unknown, mode: ThemeColorMode): ThemeColorValue {
  if (value === "") return "";
  ...
  if (mode === "truecolor") return value as `#${string}`;
  const red = Number.parseInt(value.slice(1, 3), 16);
  const green = Number.parseInt(value.slice(3, 5), 16);
  const blue = Number.parseInt(value.slice(5, 7), 16);
...
function resolvedRuntimeTheme(
  name: string,
  foreground: Readonly<Record<ThemeColor, ThemeColorValue>>,
  background: Readonly<Record<ThemeBg, ThemeColorValue>>,
): Theme {
...
export const Theme = class Theme {
  ...
  constructor(
    ...
    const foregroundTokens = THEME_TOKENS.filter(
      (token): token is ThemeColor => !THEME_BACKGROUND_TOKENS.includes(token as ThemeBg),
    );
    const unknownForeground = Object.keys(foreground).filter((token) => !foregroundTokens.includes(token as ThemeColor));
    const unknownBackground = Object.keys(background).filter((token) => !THEME_BACKGROUND_TOKENS.includes(token as ThemeBg));
    ...
    const normalizedForeground = {} as Record<ThemeColor, ThemeColorValue>;
    for (const token of foregroundTokens) {
      const selected = token === "thinkingMax" && foreground[token] === undefined
        ...
        : token === "scrollbarThumb" && foreground[token] === undefined
          ...
          : foreground[token];
      normalizedForeground[token] = normalizedRuntimeColor(selected, mode);
    }
    const normalizedBackground = {} as Record<ThemeBg, ThemeColorValue>;
    for (const token of THEME_BACKGROUND_TOKENS) {
      normalizedBackground[token] = normalizedRuntimeColor(background[token], mode);
    }
    ...
    for (const [name, value] of Object.entries(normalizedForeground) as Array<[ThemeColor, ThemeColorValue]>) {
      resolvedForeground.set(name, runtimeColorAnsi(value, false, mode));
    }
    for (const [name, value] of Object.entries(normalizedBackground) as Array<[ThemeBg, ThemeColorValue]>) {
...
) as readonly Exclude<ThemeToken, "thinkingMax" | "scrollbarThumb">[];
...
function tokenTheme(value: Record<string, unknown>, allowBuiltinName = false): ThemeDefinition {
  ...
  const tokenValue = (selected: unknown, label: string): ThemeColorValue | string => {
    ...
    if (
      ...
      && !selected.startsWith("#")
      && !selected.includes("\0")
      && Array.from(selected).length <= 1_024
    ) {
      return selected;
    }
  ...
  tokens.scrollbarThumb = colors.scrollbarThumb === undefined
    ? tokens.borderAccent
