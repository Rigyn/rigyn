...
/** rigyn actions contributed to the shared TUI keybinding registry. */
...
export interface KeybindingConflict {
  scope: "editor" | "selection" | "transcript" | "session" | "models" | "tree";
  key: string;
...
const TRANSCRIPT_ACTIONS = KEYBINDING_ACTIONS.filter((action) => action.startsWith("tui.transcript.")) as KeybindingAction[];
...
function managerConfig(overrides: KeybindingOverrides): KeybindingsConfig {
  ...
  for (const action of KEYBINDING_ACTIONS) {
    ...
    if (selected !== undefined) config[action] = bindingArray(selected, action) as KeybindingsConfig[typeof action];
  }
  return config;
}
...
export class KeybindingsManager extends TerminalKeybindingsManager {
  ...
  constructor(
    ...
    super(
      usesDefinitions ? selected as KeybindingDefinitions : COMPLETE_KEYBINDING_DEFINITIONS,
      usesDefinitions
        ? userBindings
        : managerConfig(selected as KeybindingOverrides),
    );
  }
