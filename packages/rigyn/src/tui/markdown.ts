...
function delimiterCharacter(value: string | undefined): { punctuation: boolean; whitespace: boolean } {
  return {
...
function inlineMarkdownSpans(source: string, baseRole?: ThemeRole, depth = 0): MarkdownSpan[] {
  if (depth >= MAX_INLINE_DEPTH) return source === "" ? [] : [{ text: source, ...(baseRole === undefined ? {} : { role: baseRole }) }];
