...
export interface RuntimeExtensionListenerContext {
  ...
  /** Ordered model cycle scope for the current session. */
...
function sharedEventPayload(value: unknown): { payload: JsonValue; bytes: number } {
  ...
  return { payload: JSON.parse(snapshot.serialized) as JsonValue, bytes: snapshot.bytes };
}
...
const RUNTIME_HOST_IMPORTS = new Map<string, string>([
  ...
  ["rigyn/serve", new URL(`../serve/index${runtimeHostModuleExtension}`, import.meta.url).href],
...
export function runtimeSessionGuardResult(value: unknown): RuntimeSessionGuardResult {
  const selected = runtimeSessionRecord(
    value,
    ["cancel", "reason"],
    "Runtime session guard result",
  );
  if (selected.cancel !== undefined && typeof selected.cancel !== "boolean") {
  ...
  return {
    ...(selected.cancel === undefined ? {} : { cancel: selected.cancel }),
    ...(selected.reason === undefined
      ? {}
      : { reason: bounded(selected.reason, "Runtime session cancellation reason", 16 * 1024) }),
  };
}
...
function canonicalRuntimeUserShellResult(value: unknown, label = "Runtime user-shell result"): RuntimeUserShellResult {
  ...
  const cancelled = typeof result.cancelled === "boolean" ? result.cancelled : undefined;
  const timedOut = typeof result.timedOut === "boolean" ? result.timedOut : undefined;
  ...
  if (result.fullOutputPath !== undefined && typeof result.fullOutputPath !== "string") {
...
function canonicalMessages(value: unknown, label: string): CanonicalMessage[] {
  ...
  return messages as CanonicalMessage[];
}

function runtimeConstrainedSampling(
  value: unknown,
  label: string,
  ...
  if (tagged.type === "grammar") {
    ...
    const variant = (selected: unknown, variantLabel: string): string | undefined => {
    ...
    return {
      type: "grammar",
      variants: {
        ...(openaiLark === undefined ? {} : { openai_lark: openaiLark }),
        ...(openaiRegex === undefined ? {} : { openai_regex: openaiRegex }),
      },
    };
  }
...
function runtimeProviderTools(value: unknown, label: string): ProviderToolDefinition[] {
  ...
  const tools = value.map((item, index) => {
    ...
    return {
      ...
      ...(record.constrainedSampling === undefined
        ? {}
        : {
      ...
      ...(record.loading === undefined
        ? {}
        : { loading: record.loading as NonNullable<ProviderToolDefinition["loading"]> }),
...
class RuntimeHarnessTool implements HarnessTool {
  ...
  constructor(
    ...
    this.definition = {
      ...
      ...(registration.constrainedSampling === undefined
        ? {}
        : { constrainedSampling: registration.constrainedSampling }),
...
function directToolDetails(value: unknown): JsonValue | undefined {
  if (value === undefined) return undefined;
  try {
    return cloneBounded(value, "Extension tool details", MAX_TOOL_RESULT_METADATA_BYTES) as JsonValue;
  } catch {
    return undefined;
  }
}
...
function directToolRegistration<TParams extends TSchema, TDetails, TState>(
  ...
  return {
    ...
    ...(tool.resources === undefined
      ? {}
      : { resources: (input, context) => tool.resources!(input as never, context) }),
...
function directComponentBlock(output: DirectToolRenderOutput, width: number): RuntimeUiBlock {
  if (output !== null && typeof output === "object" && typeof (output as Component).render === "function") {
    return {
      lines: (output as Component).render(width).map((line) => ({ spans: [{ text: line }] })),
    };
  }
  if (output !== null && typeof output === "object" && Array.isArray((output as RuntimeUiBlock).lines)) {
    return output as RuntimeUiBlock;
  }
...
function directToolRenderer<TParams extends TSchema, TDetails, TState>(
  ...
  const component = (output: DirectToolRenderOutput): DisposableComponent | undefined =>
    output !== null && typeof output === "object" && typeof (output as Component).render === "function"
      ? output as DisposableComponent
      : undefined;
  ...
  const disposeState = (state: DirectToolRendererState): void => {
    let failure: unknown;
    try {
  ...
  const selectedState = (callId: string): DirectToolRendererState => {
    ...
    if (state === undefined) {
      if (states.size >= MAX_DIRECT_TOOL_RENDERER_STATES) {
        const oldest = states.entries().next().value as [string, DirectToolRendererState] | undefined;
        if (oldest !== undefined) {
          states.delete(oldest[0]);
          disposeState(oldest[1]);
        }
      }
      state = { state: {} };
  ...
  const renderContext = (
    ...
    ...(view.result?.status === undefined ? {} : { resultStatus: view.result.status }),
    ...(view.result?.summary === undefined ? {} : { resultSummary: view.result.summary }),
    ...(view.result?.nextActions === undefined ? {} : { resultNextActions: view.result.nextActions }),
  });
  const renderResult = (
    view: Readonly<RuntimeToolRenderView>,
    context: RuntimeUiRenderContext,
    ...
    const content = directContent === undefined
      ? (() => {
          ...
          return typeof publicContent === "string" ? [{ type: "text" as const, text: publicContent }] : publicContent;
        })()
  ...
  return {
    ...
    [DIRECT_TOOL_RENDER_RESULT](view, content, context, bridge) {
      return tool.renderResult === undefined ? undefined : renderResult(view, context, bridge, content);
    },
...
function activation(
    ...
    on(topicValue, handler) {
      ...
      const dispose = (): void => {
        ...
        if (!staged.committed) {
          ...
          const disposerIndex = staged.disposers.indexOf(dispose);
      ...
      if (!staged.committed) {
        ...
        if (eventBus !== undefined) staged.disposers.push(dispose);
        return dispose;
      }
      if (eventBus === undefined) {
        ...
        try {
          ...
          host.registerLiveDisposer(staged.entry, staged.generation, dispose);
        } catch (cause) {
          dispose();
          throw cause;
        }
  ...
  const registerRuntimeTool = (tool: RuntimeToolRegistration): boolean => {
    assertActive();
    ...
    if (inputSchemaSnapshot.value === null || typeof inputSchemaSnapshot.value !== "object"
      || Array.isArray(inputSchemaSnapshot.value)) {
      throw new Error("Runtime tool inputSchema must be a JSON object");
    }
    const inputSchema = JSON.parse(inputSchemaSnapshot.serialized) as Record<string, JsonValue>;
    ...
    if (staged.committed) return host.registerLiveTool(staged.entry, staged.generation, registration);
  ...
  const registerRuntimeCommand = (name: string, command: RuntimeDirectCommandRegistration): void => {
    ...
    if (typeof handler !== "function") throw new Error("Runtime command handler must be a function");
    const registration: RuntimeCommandRegistration = {
      ...
      execute(context) {
        ...
        return handler.call(command, args, Object.freeze(directContext));
      },
    };
  ...
  const directApi: ExtensionAPI = {
    ...
    registerTool(tool) {
      const accepted = registerRuntimeTool(directToolRegistration(tool));
      if (accepted) {
        compatibilityProjection.tools.set(tool.name, {
          definition: tool as never,
          sourceInfo: compatibilitySourceInfo,
        });
      } else compatibilityProjection.tools.delete(tool.name);
      if (!accepted) return;
      const renderer = directToolRenderer(tool, workspace);
      if (!staged.committed) {
    ...
    registerMarkdownTransformer(transformer) {
      assertActive();
      if (typeof transformer !== "function") throw new Error("Runtime Markdown transformer is invalid");
      if (staged.committed) {
...
export class RuntimeExtensionHost {
  ...
  transformMarkdown(markdown: string, context: Readonly<MarkdownTransformContext>): string {
    ...
    if (
      ...
      || typeof safeContext.isStreaming !== "boolean"
  ...
  directActions(
    ...
    } as RuntimeDirectActionsHandler;
  }
  ...
  setAdvancedUiHandler(handler: RuntimeAdvancedUiHostHandler | undefined): void {
    ...
    for (const operation of [...this.#initialAdvancedUi]) {
      try {
  ...
  unregisterLiveToolRenderer(
    ...
    this.#assertLive(entry, generation);
    const prior = this.#toolRenderers.get(name);
    if (prior === undefined || prior.generation !== generation || ownerKey(prior.entry) !== ownerKey(entry)) return;
  ...
  #generationBoundView<T extends object>(
    ...
    value: T,
    assertCurrent?: () => void,
    project?: (property: PropertyKey, selected: unknown) => unknown,
  ): T {
    ...
    const read = (property: PropertyKey): unknown => {
      assertLive();
      const selected: unknown = Reflect.get(value, property, value);
      if (typeof selected !== "function") return project?.(property, selected) ?? selected;
      const existing = methods.get(property);
      if (existing !== undefined) return existing;
      const guarded = (...args: unknown[]): unknown => {
        assertLive();
        return Reflect.apply(selected, value, args);
      };
      methods.set(property, guarded);
      return guarded;
    };
    const facade = Object.create(Object.getPrototypeOf(value)) as T;
  ...
  #generationBoundCall<TArgs extends unknown[], TResult>(
    owned: Pick<OwnedListener, "entry" | "generation">,
  ...
  #directUiContext(
    ...
    const bound = (this.#directUiHandler ?? this.#sessionUiHandler)?.(
  ...
  #directModelRegistry(
    ...
    const guardedAuth = <T extends object>(auth: T): T => {
      ...
      const guarded = this.#generationBoundView(owned, auth, assertCurrent, (_property, selected) =>
        selected !== null && typeof selected === "object" ? guardedAuth(selected) : selected);
    ...
    const guardedProvider = (provider: ExtensionProvider): ExtensionProvider => {
      ...
      const guarded = this.#generationBoundView(owned, provider, assertCurrent, (property, selected) =>
        property === "auth" && selected !== null && typeof selected === "object"
  ...
  #listenerContext(
    ...
    const uiOwner = runtimeUiOwner(owned.entry, owned.generation);
    const interactive = this.#interactiveUiHandler?.(owned.entry.extensionId, signal, uiOwner.ownerKey);
    ...
    return Object.freeze({
      ...
      ui: this.#generationBoundView(
        owned,
        this.#directUiContext(owned, signal, ui, interactive !== undefined),
        assertCurrent,
      ),
      ...
      scopedModels: Object.freeze((direct.scopedModels ?? []).map((entry) => Object.freeze({
        model: immutableExtensionModel(modelRegistry.present(entry.model)),
  ...
  async reduceBeforeUserShell(
    ...
    for (const owned of this.#listeners.get("user_bash") ?? []) {
      ...
      try {
        ...
        const result = await withAbort(Promise.resolve(listener(
        ...
        if (result.result !== undefined) {
          if (typeof result.result.output !== "string" || typeof result.result.cancelled !== "boolean"
            ...
            || (result.result.isError !== undefined && typeof result.result.isError !== "boolean")
            || (result.result.timedOut !== undefined && typeof result.result.timedOut !== "boolean")
            || (result.result.signal !== undefined && typeof result.result.signal !== "string")
            || (result.result.exitCode !== undefined && !Number.isSafeInteger(result.result.exitCode))
            || (result.result.fullOutputPath !== undefined && typeof result.result.fullOutputPath !== "string")) {
          ...
          const terminal = normalizeShellTerminalState({
            ...(result.result.exitCode === undefined ? {} : { exitCode: result.result.exitCode }),
            ...(result.result.isError === undefined ? {} : { isError: result.result.isError }),
            cancelled: result.result.cancelled,
            ...(result.result.timedOut === undefined ? {} : { timedOut: result.result.timedOut }),
            ...(signal === undefined ? {} : { signal }),
          });
  ...
  async reduceBeforeAgentStart(event: RuntimeBeforeAgentStartEvent, signal?: AbortSignal): Promise<RuntimeBeforeAgentStartReduction> {
    ...
    const snapshot = (systemPrompt: string): RuntimeNativeSystemPromptSnapshot => {
      ...
      return Object.freeze({
        ...
        ...(selectedComposition === undefined ? {} : { composition: selectedComposition }),
      });
    };
    const reduced = await this.#reduce("before_agent_start", {
      ...
      const result = await this.#currentSystemPrompt.run(
        ...
        async () => await (listener as (value: unknown) => Promise<unknown>)({
          ...
          ...(currentSnapshot.composition === undefined
            ? {}
            : { promptComposition: structuredClone(currentSnapshot.composition) }),
  ...
  async applyBeforeProviderRequestPayload(
    ...
    for (const owned of this.#listeners.get("before_provider_request") ?? []) {
      ...
      try {
        ...
        current = cloneBounded(selected, "Direct provider request replacement") as JsonValue;
      } catch (cause) {
        if (signal?.aborted === true) throw abortError(signal);
        this.#recordListenerFailure(owned, cause);
      }
  ...
  async reduceToolCall(event: RuntimeToolCallEvent, signal?: AbortSignal): Promise<RuntimeToolCallReduction> {
    ...
    for (const owned of this.#listeners.get("tool_call") ?? []) {
      ...
      const inputSnapshot = JSON.parse(boundedInput.serialized) as JsonValue;
      ...
      if (selected.block === true) {
        ...
        return {
          ...
          ...(selected.terminate === true ? { terminate: true } : {}),
        };
      }
    }
    return {
  ...
  async reduceToolResult(event: RuntimeToolResultEvent, signal?: AbortSignal): Promise<ToolResult> {
    ...
    const reduced = await this.#reduce("tool_result", initial, async (current, listener) => {
      ...
      const selectedDetails = selectedDetailsSnapshot === undefined
        ? undefined
        : JSON.parse(selectedDetailsSnapshot.serialized) as JsonValue;
      ...
      const result: ToolResult = validateResult({
        ...
        ...(selectedDetails === undefined ? {} : { metadata: selectedDetails }),
  ...
  async reduceSessionBeforeTree(event: RuntimeSessionBeforeTreeEvent, signal?: AbortSignal): Promise<RuntimeTreeResult> {
    ...
    return await this.#reduce("session_before_tree", {} as RuntimeTreeResult, async (current, listener) => {
      ...
      if (selectedSummary !== undefined) {
        if (typeof selectedSummary.summary !== "string" || selectedSummary.summary.trim() === "") {
          throw new Error("Runtime tree summary must be a non-empty string");
        }
        bounded(selectedSummary.summary, "Runtime tree summary", MAX_RUNTIME_TREE_SUMMARY_BYTES);
        ...
        selectedSummaryDetails = detailsSnapshot === undefined
          ? undefined
          : JSON.parse(detailsSnapshot.serialized) as JsonValue;
      }
      if (selected.customInstructions !== undefined) {
        if (typeof selected.customInstructions !== "string") throw new Error("Runtime tree instructions must be a string");
        bounded(selected.customInstructions, "Runtime tree instructions", MAX_RUNTIME_TREE_INSTRUCTIONS_BYTES);
      }
      if (selected.replaceInstructions !== undefined && typeof selected.replaceInstructions !== "boolean") {
      ...
      const value: RuntimeTreeResult = cloneBounded({
        ...
        ...(selectedSummary === undefined ? {} : {
          summary: {
            summary: selectedSummary.summary as string,
            ...(selectedSummaryDetails === undefined ? {} : { details: selectedSummaryDetails }),
            ...(selectedSummary.usage === undefined
              ? {}
              : { usage: canonicalUsage(selectedSummary.usage as import("@rigyn/kernel").Usage) }),
          },
        }),
        ...(selected.customInstructions === undefined ? {} : { customInstructions: selected.customInstructions }),
        ...(selected.replaceInstructions === undefined ? {} : { replaceInstructions: selected.replaceInstructions }),
  ...
  async reduceSessionBeforeCompact(event: RuntimeSessionBeforeCompactEvent): Promise<RuntimeSessionBeforeCompactResult> {
    ...
    return await this.#reduce("session_before_compact", {} as RuntimeSessionBeforeCompactResult, async (current, listener) => {
      ...
      if (selectedCompaction !== undefined) {
        if (typeof selectedCompaction.summary !== "string") {
          throw new Error("Runtime compaction summary must be a string");
        }
        if (typeof selectedCompaction.firstKeptEntryId !== "string") {
          throw new Error("Runtime compaction first kept entry must be a string");
        }
        bounded(selectedCompaction.summary, "Runtime compaction summary", 4 * 1024 * 1024);
        bounded(selectedCompaction.firstKeptEntryId, "Runtime compaction first kept entry", 1_024);
        if (!Number.isSafeInteger(selectedCompaction.tokensBefore) || (selectedCompaction.tokensBefore as number) < 0) {
      ...
      const value: RuntimeSessionBeforeCompactResult = cloneBounded({
        ...(selected.cancel === undefined ? {} : { cancel: selected.cancel }),
        ...(selectedCompaction === undefined ? {} : {
          compaction: {
            summary: selectedCompaction.summary as string,
            firstKeptEntryId: selectedCompaction.firstKeptEntryId as string,
            tokensBefore: selectedCompaction.tokensBefore as number,
            ...(selectedCompactionDetails === undefined ? {} : { details: selectedCompactionDetails }),
            ...(selectedCompaction.usage === undefined
              ? {}
              : { usage: canonicalUsage(selectedCompaction.usage as import("@rigyn/kernel").Usage) }),
          },
        }),
      }, "Runtime compaction result");
      return { value, stop: value.cancel === true };
  ...
  async #reduceGuard<K extends "session_before_switch" | "session_before_fork">(
    ...
    return await this.#reduce(eventName, {} as RuntimeSessionGuardResult, async (current, listener, entry) => {
  ...
  async #close(): Promise<void> {
    ...
    // Each category gets one fresh phase budget. This keeps a hung process
    // cleanup from starving disposers, or a hung live-registration cleanup
    // from preventing module-loader cleanup. The asynchronous upper
    // bound is four sequential shutdownTimeoutMs phases, independent of
    // callback count.
    this.#closed = true;
