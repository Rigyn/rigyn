...
async function readPackageManifest(
  ...
  const packageConfig = packageJson?.value.rigyn;
  ...
  async #acquireOperationLock(signal?: AbortSignal): Promise<() => Promise<void>> {
    ...
    while (true) {
      ...
      try {
        ...
        return async () => {
          try {
            const current = JSON.parse(await readOperationLease(path)) as { token?: unknown };
      ...
      try {
        const current = JSON.parse(await readOperationLease(path)) as {
