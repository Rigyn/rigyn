...
async function isNpmCli(candidate: string | undefined): Promise<boolean> {
  ...
  try {
    ...
    return manifest.name === "npm" && manifest.bin?.npm === "bin/npm-cli.js";
  } catch {
    return false;
  }
}
...
export async function packExtensionPackage(source: string, destination: string): Promise<{
  ...
  const temporary = await mkdtemp(join(tmpdir(), "rigyn-pack-publish-"));
  ...
  try {
    ...
    publicationDirectory = await mkdtemp(join(canonicalDestination, ".rigyn-pack-publish-"));
