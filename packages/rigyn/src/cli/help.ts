...
const header = `rigyn ${RIGYN_VERSION} — agent harness with read, bash, edit, write, grep, find, and ls tools`;
...
Commands:
  ...
  rigyn logs [--json]            Locate private local operational logs
  rigyn stats [--json]           Summarize local aggregate usage and failures
  rigyn completions bash|zsh|fish
                                 Generate a static shell completion script
  rigyn serve                    Run the authenticated HTTP and SSE service
  ...
  rigyn config validate [--scope user|project]
                                 Validate settings without changing files
  rigyn self-install             Install a source-built private user copy
  rigyn self-update              Update that copy; standalone prints its installer command
  rigyn uninstall --yes          Fully remove the installed copy and its managed state
...
Builds or installs an independent source-built copy under ~/.rigyn without
...
For a source-built private installation, downloads the latest verified rigyn
...
Usage:
  ...
  rigyn extensions author validate|inspect|smoke|refresh|report PACKAGE
  ...
  rigyn extensions remove SOURCE [-l]
  rigyn extensions update SOURCE [-l] [--allow-scripts]
...
Usage:
  ...
  rigyn config edit [--scope user|project]
  rigyn config validate [--scope user|project] [--json]
...
workspace trust. Persistent settings live in ~/.rigyn/config.json and
WORKSPACE/.rigyn/config.json.
`,
  diagnostics: `${header}
...
Rigyn never sends these logs or aggregate metrics to a remote service.
`,
  stats: `${header}
...
Usage:
  rigyn sessions doctor [--json] [--all] [--workspace DIR] [--session-dir DIR]
...
Usage:
  rigyn serve [--host HOST] [--port PORT] [--workspace DIR] [--session-dir DIR]
              [--approve | --no-approve] [--offline] [--no-extensions]
              [--extension PATH ...]

Runs an authenticated HTTP and SSE service over the canonical agent runtime.
The default address is 127.0.0.1:4317. Set RIGYN_SERVE_TOKEN to a secret with
...
Load it in the current shell:
  Bash: source <(rigyn completions bash)
  Zsh:  source <(rigyn completions zsh)
  Fish: rigyn completions fish | source
`,
});
