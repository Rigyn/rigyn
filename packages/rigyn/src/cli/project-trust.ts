...
import { sharedWorkspaceAgentSkillDirectories } from "../context/skill-roots.js";
...
const SHARED_USER_DIRECTORIES = new Set<string>([
  ".agents/skills",
  ".claude/skills",
  ".codex/skills",
]);
...
export async function discoverProjectTrustResources(
  ...
  const home = canonicalizePath(resolve(
    homeDirectory.trim().length > 0 && isAbsolute(homeDirectory) ? homeDirectory : homedir(),
  ));
  ...
  for (const relativePath of PROJECT_DIRECTORIES) {
    if (relativePath === ".agents/skills") {
      for (const directory of sharedWorkspaceAgentSkillDirectories(canonical, home)) {
        if (await directoryContainsResource(directory)) {
          resources.push(relative(canonical, directory).replaceAll("\\", "/"));
        }
      }
      continue;
    }
    if (canonical === home && SHARED_USER_DIRECTORIES.has(relativePath)) continue;
...
export class ProjectTrustResolver {
  ...
  async #resolve(canonical: string): Promise<boolean> {
    ...
    if (resources === undefined) {
      resources = await discoverProjectTrustResources(canonical, this.#homeDirectory, this.#agentDirectory);
