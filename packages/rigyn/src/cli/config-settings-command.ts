...
function normalizedSettings(value: string): string {
  ...
  SettingsManager.inMemory(parsed as never);
...
async function missingSettingsDocument(scope: "user" | "project"): Promise<string> {
  ...
  const schema = (JSON.parse(template) as { $schema?: unknown }).$schema;
  if (typeof schema !== "string" || schema === "") throw new Error("The packaged config template has no schema URI");
...
export async function inspectSettingsConfig(
  ...
  const path = scope === "user" ? join(agentDirectory, "config.json") : join(workspace, ".rigyn", "config.json");
  let exists = false;
  try {
...
export async function runSettingsConfigCommand(
  ...
  const path = scope === "user" ? paths.settings : join(workspace, ".rigyn", "config.json");
  ...
  if (scope === "project" && projectConfigRootMatchesAgentDir(workspace, paths.agentDirectory)) {
    throw new Error(`Project settings cannot be ${action === "path" ? "reported" : action === "edit" ? "edited" : "validated"} because this workspace's .rigyn directory is the active Rigyn home`);
  }
  if (action === "path") {
  ...
  const updated = await edit(initial, {
    environment,
    cwd: workspace,
    ...(editorCommand === undefined ? {} : { command: editorCommand }),
