...
The external backend governs only model tool calls on an `AgentSession` configured with that backend. Trusted runtime extensions are JavaScript loaded into the host process, and `rigyn.exec` is explicit extension process authority; neither is silently presented as sandboxed or routed through the tool backend.
