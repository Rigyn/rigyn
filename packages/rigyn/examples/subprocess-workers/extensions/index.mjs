...
function workerArguments(cliEntry, agent, task) {
  const specialistPolicy = [
    `Act as the ${agent.name} specialist in a bounded delegated run.`,
    agent.instructions,
  ...
  return [
    ...
    "--max-steps", "24",
    "--thinking", "low",
    "--append-system-prompt", specialistPolicy,
    `Task: ${task}`,
  ];
}
...
async function managedEcho(rigyn, text, cwd, signal) {
  const id = rigyn.processes.spawn({
    argv: [process.execPath, "--eval", "process.stdout.write(process.argv[1] ?? '')", "--", text],
    cwd,
    signal,
    timeoutMs: 10_000,
    stdin: "ignore",
    stdout: "capture",
    stderr: "capture",
    captureLimitBytes: MAX_TASK_BYTES,
  });
  const result = await rigyn.processes.wait(id, { signal });
  if (result.state !== "succeeded" || result.exitCode !== 0) {
    const detail = Buffer.from(result.stderr).toString("utf8").trim();
    throw new Error(detail || `managed process ended in state ${result.state}`);
  }
...
export default async function activate(rigyn) {
  ...
  rigyn.registerCommand("example-process", {
    description: "Run one generation-owned shell-free process with bounded capture",
    async handler(args, context) {
      const text = args.trim();
      if (text === "" || Buffer.byteLength(text, "utf8") > MAX_TASK_BYTES) {
        context.ui.notify(`Usage: /example-process TEXT (up to ${MAX_TASK_BYTES} bytes)`, "warning");
        return;
      }
      context.ui.notify(await managedEcho(rigyn, text, context.cwd, context.signal), "info");
    },
  });
}
