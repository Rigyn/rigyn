...
async function withEnvironment<T>(value: MeasurementFixture, operation: () => Promise<T>): Promise<T> {
  const previous = {
    ...
    agentDirectory: process.env.RIGYN_HOME,
  ...
  process.env.RIGYN_HOME = join(value.configHome, "rigyn");
  ...
  try {
    ...
    if (previous.agentDirectory === undefined) delete process.env.RIGYN_HOME;
    else process.env.RIGYN_HOME = previous.agentDirectory;
...
async function refreshSample(): Promise<number> {
  const value = await fixture("rigyn-refresh-large-");
  let runtime: LoadedRuntime | undefined;
  try {
    const extensionRoot = join(value.configHome, "rigyn", "extensions");
...
function observeSessionRowMaterialization(manager: SessionManager): {
  restore(): void;
  rows(): number;
} {
...
async function coldEventPageSample(eventCount: number): Promise<ColdPageSample> {
  const value = await fixture(`rigyn-cold-page-${eventCount}-`);
  let runtime: LoadedRuntime | undefined;
  ...
  try {
    ...
    return await withEnvironment(value, async () => {
      ...
      if (
        ...
        first.message.content?.[0]?.text !== "Deterministic resume fixture message 0"
      ) {
