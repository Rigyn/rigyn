...
function diagnosticError(value: unknown): NonNullable<AssistantMessageDiagnostic["error"]> {
  ...
  return {
    ...
    ...(selected.stack === undefined ? {} : { stack: text(selected.stack, "Assistant diagnostic error stack", MAX_ERROR_TEXT_BYTES) }),
    ...(code === undefined
      ? {}
      : { code: typeof code === "string" ? text(code, "Assistant diagnostic error code", MAX_TYPE_BYTES) : code }),
