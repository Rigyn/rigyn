...
test("relay forwards one bounded request without inheriting the host environment", async (t) => {
  ...
  assert.deepEqual(
    response.result.metadata.environment.filter((name: string) => name !== "NODE_V8_COVERAGE"),
    [],
  );
...
test("relay cancellation reaps its executor process tree", async (t) => {
  ...
  await assert.rejects(relayed, /terminated/u);
...
test("relay kills and reaps an executor whose response exceeds its bound", async (t) => {
  ...
  await writeFile(executor, `
    ...
    process.stdout.write(Buffer.alloc(${MAX_RESPONSE_BYTES + 1}, 120));
    setInterval(() => {}, 1000);
  `);
  ...
  await assert.rejects(relayed, /response exceeds/u);
