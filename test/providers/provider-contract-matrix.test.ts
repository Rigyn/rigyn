...
function mistralSse(type: string, value: Record<string, unknown>): Uint8Array {
...
function mistralConversationChunks(): Uint8Array[] {
  return [
    mistralSse("conversation.response.started", { conversation_id: "conversation-contract" }),
    mistralSse("message.output.delta", {
      output_index: 0,
      content_index: 0,
      id: "message-contract",
      content: { type: "text", text: "ok" },
    }),
    mistralSse("function.call.delta", {
      output_index: 1,
      id: "function-contract",
      tool_call_id: "contract-call",
      name: "lookup",
      arguments: '{"value":1}',
    }),
    mistralSse("conversation.response.done", {
      usage: {
        prompt_tokens: 1_000,
        completion_tokens: 100,
        total_tokens: 1_100,
...
const fixtures: WireFixture[] = [
  ...
  {
    name: "Mistral Chat Completions mode",
    provider: "mistral",
  ...
  {
    name: "Mistral Conversations mode",
    provider: "mistral",
    create: (fetch) => new MistralConversationsAdapter({ apiKey: "offline", store: false, fetch }),
    chunks: mistralConversationChunks,
    contentType: "text/event-stream",
    state: () => ({
      kind: "mistral_conversations",
      model: "test-model",
      requestFingerprint: "offline-fingerprint",
      outputs: [{
        object: "entry",
        type: "message.output",
        id: "state-message",
        role: "assistant",
        content: [{ type: "text", text: "state-sentinel" }],
      }],
    }),
  },
  {
...
test("offline native-provider request and stream contract matrix", async (t) => {
  for (const fixture of fixtures) {
    await t.test(fixture.name, async () => {
      ...
      const providerRequest = contractRequest(fixture.provider);
    ...
    await t.test(fixture.name, async (providerTest) => {
      for (const phase of phases) {
        await providerTest.test(phase.name, async () => {
          ...
          for await (const event of adapter.stream(contractRequest(fixture.provider), controller.signal)) {
...
function contractRequest(provider: ProviderId): ProviderRequest {
  ...
  return {
    provider,
    model: "test-model",
    messages: projectMessagesForProvider(messages, provider),
    tools: [],
  };
}
