...
test("comparative live runner applies one task contract to both configured harnesses", async () => {
  ...
  try {
    ...
    await writeFile(agent, `
      ...
      const usage = { inputTokens: 10, outputTokens: 2, totalTokens: 12, cacheReadTokens: 3, cacheWriteTokens: 0, reasoningTokens: 0, cost: "0.01" };
      if (format === "rigyn") {
        const events = [
        ...
        writeSync(1, JSON.stringify({ completed: true, steps: 1, toolErrors: 0, usage: { ...usage, costUsd: usage.cost } }) + "\\n");
      }
    `);
