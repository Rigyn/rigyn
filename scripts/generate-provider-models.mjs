...
export function validateMaintainedCatalog(models) {
  if (!Array.isArray(models) || models.length === 0) {
    throw new Error("rigyn's maintained catalog must contain at least one model");
  }
  const keys = new Set();
...
function projectMaintainedModel(model) {
  ...
  return {
    ...
    ...(directCompatibility === undefined ? {} : { compat: directCompatibility }),
  };
}
...
export function projectMaintainedModels(models) {
  ...
  if (projected.length === 0) throw new Error("rigyn's strict direct-model projection is empty");
  return projected;
}
