...
function packageInputPath(directory, path) {
  assert.equal(typeof path, "string", `npm pack reported a non-string path for ${directory}`);
  const components = path.split("/");
  assert.ok(
    path !== ""
      && !path.startsWith("/")
...
async function stage(output, sourceRef) {
  ...
  try {
    ...
    for (const { name, directory } of RIGYN_PACKAGE_GRAPH) {
      ...
      const requiredNativeOutputs = name === "@rigyn/terminal" ? terminalNativeOutputs
        : name === "@rigyn/kernel" ? kernelNativeOutputs
          : [];
