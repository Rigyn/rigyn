...
const rigynKernelRuntimeImports = new Set();
for (const file of sourceFiles) {
  ...
  for (const match of text.matchAll(importPattern)) {
    ...
    if (owner?.manifest.name === "rigyn" && specifier.startsWith("@rigyn/kernel/runtime/")) {
      rigynKernelRuntimeImports.add(specifier);
    }
...
const reviewedKernelRuntimeModules = new Set(["@rigyn/kernel/runtime/index"]);
for (const file of sourceFiles) {
  if (!file.startsWith(`${kernelRuntimeRoot}${sep}`)) continue;
  const module = relative(kernelRuntimeRoot, file)
    .split(sep).join("/")
    .replace(/\.(?:c|m)?(?:j|t)sx?$/u, "");
  const specifier = `@rigyn/kernel/runtime/${module}`;
  if (!rigynKernelRuntimeImports.has(specifier) && !reviewedKernelRuntimeModules.has(specifier)) {
    failures.push(`${relative(repositoryRoot, file)} is exposed by @rigyn/kernel/runtime/* without a first-party import or explicit boundary review`);
  }
}
